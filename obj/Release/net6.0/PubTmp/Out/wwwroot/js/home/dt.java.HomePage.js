...
function buildFromCook(build) {
	var cookieValues = jQuery.parseJSON(Cookies.get('backCookieTMBYO'));
	var cokC = 0;
	...
	if (build === 1) {
		jQuery.each(cookieValues, function (i, e) {
			...
			cokC += 1;
			cokC === cokL ? (Cookies.set('backCookieTMBYO', '', { expires: -1 }), dateByDest(cokR)) : ''
		});
	};
};
function startAgain(obj) {
	...
	$('#dvTranspError').hide();
	Cookies.set('backCookieTMBYO', '', { expires: -1 });
};
...
function submitForm(frmNA) {
	...
	Cookies.set('backCookieTMBYO', stringQuery, { expires: 365 });
	var stringForm = ''
...
function buildTransportationFrom() {
	...
	var cookSaved = JSON.parse(Cookies.get('backCookieTMBYO'));
...
function buildTransportationError(err) {
	...
	jQuery.each(cookieValues, function (i, e) {
		...
		cokC += 1;
		cokC === cokL ? (Cookies.set('backCookieTMBYO', '', { expires: -1 }), $('#sptrancities').html(ctys), $('#perrorCode').html(err), $('.dvWaitGlobe').hide(), $('.dvtranspMainContainer').hide(), $('#dvTranspError').show()) : ''
	});
};
...
function cookieCheck() {
	selectRoomPax('1|2')
	backCookie = Cookies.get('backCookieTMBYO')
	backCookie != undefined ? (buildFromCook(1), doitArr()) : (dateByDest(), doitArr());
};
function doitArr() {
