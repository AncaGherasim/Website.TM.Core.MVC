...
function DocReady() {
    ...
    $('span[id^="adultPlus"]').click(function () { $(this).removeClass('errorClass'), adultPlus(this) });
    $('span[id^="adultMinus"]').click(function () { $(this).removeClass('errorClass'), adultMinus(this) });
    $('span[id^="childrenPlus"]').click(function () { childPlus(this) });
    $('span[id^="childrenMinus"]').click(function () { childMinus(this) });
    $('#xiAdults, input[id^="xRoom"], select[id^="xRoom"]').click(function () { $(this).removeClass('errorClass') });
...
function datePicker(dest) {
    ...
    }).click(function () { $(this).removeClass('errorClass'); });
}
// -- call transportation options
function callTransportation() {
    var paramsTrip = true;
    if ($('#qLeaveID').val() === '-1') {
        $('#qLeaveNA').addClass('errorClass');
        paramsTrip = false;
    }
    if ($('#qArrDate').val() === 'mm/dd/yyyy') {
        $('#qArrDate').addClass('errorClass');
        paramsTrip = false;
    }
    if ($('input[id^="qIDCity"]').length > 0) {
        var iTot = $('input[id^="qIDCity"]').length
        for (i = 1; i <= iTot; i++) {
            if ($('#qIDCity' + i + '').val() === '-1' && $('#qNACity' + i + '').val() == 'City name') {
                $('#qNACity' + i + '').addClass('errorClass');
                paramsTrip = false;
    ...
    if (paramsTrip == false) {
        ...
        $('#qLeaveNA').removeClass('errorClass');
        $('#qArrDate').removeClass('errorClass');
        $("[id^='qNACity']").removeClass('errorClass');
    }
            ...
            if (isNaN(Number(this.No)) === true) {
                if (this.No === 'S') {
                    ...
                    if (this.Options !== null) {
                        ...
                        dvCities = dvCities + '<div class="' + selfCss + '" id="spTransRankS' + Number(optCS + 1) + '" data-type="W" data-field="W" data-ovrnts="0"><font class="spSlegend">Transfer to ' + infocityS[1] + '</font> <img src="https://pictures.tripmasters.com/siteassets/d/self_icon.gif" align="absmiddle"/><span class="spMtransp">&#10095;</span>';
                        dvCities = dvCities + '</div>';
                ...
                if (this.Options !== null) {
                    ...
                    dvCities = dvCities + '<div class="' + selfCss + '" id="spTransRank' + ctyNo + Number(optC + 1) + '" data-type="OWN" data-field="W" data-ovrnts="0"><font class="spSlegend">Travel by</font> <img src="https://pictures.tripmasters.com/siteassets/d/self_icon.gif" align="absmiddle"/><span class="spMtransp">&#10095;</span></span>';
                    ...
                    if (this.No < totCtys) {
                        ...
                        if (DropOffStartCity != 0) {
                            if ((DropOffStartCity < this.No || DropOffStartCity == "S") && (DropOffEndCity == "E" || Number(DropOffEndCity) > this.No || this.PlaceToID == -1)) {
                                selfCss = 'spTransNoActive'
                            }
                        }
                        ...
                        dvCities = dvCities + '<div class="' + selfCss + '" id="spTransRank' + ctyNo + '1" data-type="OWN" data-field="W" data-ovrnts="0"><font class="spSlegend">Travel by</font> <img src="https://pictures.tripmasters.com/siteassets/d/self_icon.gif" align="absmiddle"/><span class="spMtransp">&#10095;</span></div>';
                        dvCities = dvCities + '<br style="clear:both"/>';
                        dvCities = dvCities + '</div>';
                    }
                }
...
function cookieCheck() {
    ...
    console.log("backCookie = " + backCookie);
    ...
    switch (typ) {
        case 'dep':
            $('#' + obj + '').autocomplete({
                ...
                $('#qLeaveNA').val('').select().removeClass('errorClass');
                $('#qLeaveNA').css("scroll-margin-top", "10px");
                document.getElementById("qLeaveNA").scrollIntoView();
        ...
        case 'arr':
            $('#' + obj + '').autocomplete({
                ...
                $('#' + obj + '').select().removeClass('errorClass');
                if ($('#' + obj + '').val() == "City name") {
                    $('#' + obj + '').val("");
                }
                $('#' + obj.replace('NA', 'ID') + '').val('-1');
                $('#' + obj + '').css("scroll-margin-top", "10px");
        ...
        if (ini > fin) {
            ...
            for (i = ini; i <= fin; i++) {
                if ($('#cityTo' + i + '').length > 0) {
                    ...
                    $('#spClose' + i + '').attr('id', 'spClose' + bfr + '');
                    $("#spClose" + bfr + "").show().unbind("click").click(function () { deletecity(Number(bfr)); });
...
function checkCarSelected(val, seq) {
    ...
    $('.dvMtravelto').each(function () {
        ...
        thisL > objSeq ? ($('#' + this.id + '').show(), $('#xFIELDCity' + thisL + '').val($('#' + this.id + '').show().find('div:visible').data('field'))) : '';
    });
}
