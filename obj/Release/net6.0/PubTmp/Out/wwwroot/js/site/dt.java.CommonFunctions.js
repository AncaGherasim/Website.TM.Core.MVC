...
$(document).ready(function () {
    ...
    if (Cookies.get('utTotal') != undefined) {
        ...
        cookVisTot = Cookies.get('utTotal');
    };
    ...
    $('#q').autocomplete({
        ...
        var $a = $('<a style="color:black;"></a>').text(suggParts[0]).attr("href", "https://" + suggParts[3].toLowerCase() + "?iSrc1=Header");
...
function setVisitViw() {
    if (utVisTotal > 0) {
        ...
        Cookies.set('utTotal', utVisTotal, { expires: 365, path: '/'});
    };
};
...
function checkCalendar() {
    var rgx = $('input').filter(function () { return this.id.match(/idLeavingFrom|qLeaveID|qReturnID|iDepCity|iDepCityTxt|xIDLeavingFrom/g )}).attr('id');
	...
	switch(rgx){
        ...
        case 'qReturnID':
            if ($('#qReturnNA').is(':visible')) {
                $('#qReturnNA').length > 0 ? (
                    $('#qReturnNA').val(town[1]),
                    $('#qReturnID').val(town[0])
                )
                : '';
            }
...
function timeBetween(date1, date2) {
    ...
    var passTime;
    passTime = days > 0 ? days + (days === 1 ? ' day ago' : ' days ago') : hours > 0 ? hours + (hours === 1 ? ' hour ago' : ' hours ago') : minutes > 0 ? minutes + (minutes === 1 ? ' minute ago' : ' minutes ago') : ('  just visited');
    return passTime
};
