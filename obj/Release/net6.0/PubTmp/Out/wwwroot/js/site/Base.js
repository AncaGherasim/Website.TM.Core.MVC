...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            msg = data;
            if (msg != '') {
                vacTxt = '<div style="padding:2px 2px;" ><a href="https://www.tripmasters.com/vacation-packages">Vacation Packages</a>&nbsp;|&nbsp;';
                ...
                for (i = 0; i <= strPrts.length - 1; i++) {
                    ...
                    switch (echP[5]) {
                        case '243':
                            tmpath = "europe/" //"http://www.tripmasters.com/europe";				
                            break;
                        case '595':
                            tmpath = "asia/" //"http://www.tripmasters.com/asia";				
                            break;
                        case '182':
                            tmpath = "latin/" //"http://www.tripmasters.com/latin";				
                            break;
                    };
                    var SiteNameCMS = "https://www.tripmasters.com/";
                    if (echP[4] == 1) {
                        if (i > 0) {
                            vacTxt = vacTxt + '&nbsp;|&nbsp;';
                            tourTxt = tourTxt + '&nbsp;|&nbsp;';
                        }
                        vacTxt = vacTxt + '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/vacations">' + echP[1] + ' Vacations:</a>&nbsp;';
                        vacTxt = vacTxt + '<a href="' + SiteNameCMS + tmpath + echP[3].replace(/\s/g, '_').toLowerCase() + '/vacations" >' + echP[3] + '</a>';

                        tourTxt = tourTxt + '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/vacations">' + echP[1] + ':</a>&nbsp;' +
                            '<a href="' + SiteNameCMS + tmpath  + echP[1].replace(/\s/g, '_').toLowerCase() + '/tours">' + echP[1] + ' Tours</a>, ' +
                            '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/trips'  + '">Trips to ' + echP[1] + '</a>, ' +
                            '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/holidays">' + echP[1] + ' Holidays</a>, ' +
                            '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/travel">' + echP[1] + ' Travel</a>, ' +
                            '<a href="' + SiteNameCMS + tmpath + echP[1].replace(/\s/g, '_').toLowerCase() + '/visit' + '">Visit ' + echP[1] + '</a>';
                    }
                }
...
function buildRecentlyViewedBase(obj) {
    ...
    $.each(obj, function () {
        ...
        if (objC < 3) {
            ...
            newVisit = newVisit + '<li class="liMostPop">';
            ...
            newVisit = newVisit + '</div>';
            newVisit = newVisit + '<div >';
            newVisit = newVisit + '<div class="colTitle">';
            newVisit = newVisit + '<a href="' + siteURL + '/' + this.STR_PlaceTitle.replace(/ /g, '_').toLowerCase() + '/' + this.PDL_Title.replace(/ /g, '_').toLowerCase() + '/package-' + this.PDLID + '">';
            newVisit = newVisit + this.PDL_Title;
            newVisit = newVisit + '</a>';
            newVisit = newVisit + '</div>'
            newVisit = newVisit + '<div class="Text_12_GrayLight dvSuggestPrice" > Viewed ' + lastVst + '</span>';
            newVisit = newVisit + '</div>'
            newVisit = newVisit + '</div>';
        ...
        success: function (data) {
            ...
            if (msg.length > 0) {
                ...
                for (i = 0; i < msg.length; i++) {             
                    ...
                    relTxt = relTxt + '<li class="liMostPop">';
                    relTxt = relTxt + '<div>';
                    relTxt = relTxt + '<a href="' + siteURL + '/' + pack.CountryNA.replace(/\s/g, '_').toLowerCase() + '/' + pack.PDL_Title.replace(/\s/g, '_').toLowerCase() + '/package-' + pack.PDLID + '">';
                    ...
                    relTxt = relTxt + '</a>';
                    relTxt = relTxt + '<p class="Text_12_GrayLight dvSuggestPrice">' + pack.Duration + ' nights from  <span class="spSuggestPrice"> $' + pack.STP_Save + '*</span></p>';
                    relTxt = relTxt + '</div>';
                    relTxt = relTxt + '</li>';
                }
