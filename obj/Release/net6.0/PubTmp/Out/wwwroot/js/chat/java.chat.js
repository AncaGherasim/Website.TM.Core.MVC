...
function getIframe() {
    ...
    if (divChatWrapper[0].children.length == 3) {
        var oButtons = divChatWrapper[0].lastChild.getElementsByTagName("button");
        oButtons[0].addEventListener("click", clickEndChat, false);
        console.log("divChatWrapper button innerText = " + oButtons[0].innerText);
    }

    var timer2 = setInterval(function () {
        if (divChatWrapper[0].children.length == 3) {
            var oButtons = divChatWrapper[0].lastChild.getElementsByTagName("button");
            let countButtons = oButtons[0].children.length;
            if (countButtons > 0) {
                let firstChild = oButtons[0].firstChild;
...
$(function () {
    ...
    function OpenChat() {
        ...
        amazon_connect('authenticate', function (callback) {
            ...
            //xobj.open('POST', " https://ilnou9d78d.execute-api.us-east-1.amazonaws.com/development/connect/chat", true);
