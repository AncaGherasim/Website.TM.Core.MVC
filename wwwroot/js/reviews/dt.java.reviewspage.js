...
function callReviewData(pg) {
	...
	options.success = function (data) {
		...
		jQuery.each(rvData, function (rvData) {
			...
			nwDv = nwDv + '<div class="dvEachRvwLeftFLex">';
			...
			if (this.TCF_FeedbackReceivedTime != null) {
				...
				if (yyyy > 2000) {
					...
					nwDv = nwDv + '<span class="spEaReLeDesc">Travel Date: ' + today + '</span>'
				}
			}

			if (rvType == 1) {
				var cC = 0;
				nwDv = nwDv + '<span class="spEaReLeLine">Travel to: <br/> <a href="' + rvLink + this.CountryNA.toLowerCase() + "/" + this.PDL_Title.replace(/\s/g, '_').toLowerCase() + '/feedback-' + this.PCC_PDLID + '">' + this.PDL_Title + '</a></span>';
				...
				nwDv = nwDv + '<span class="spEaReLeLine"><a href="' + rvLink + '">Travel to: ' + rvDest + '</a></span> </div>';
			};
