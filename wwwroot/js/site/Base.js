...
// *** Packages on Header *** //
    ...
    function getMostPop() {
        var mostPop = getItemF("objTMMostPop");
        if (mostPop !== null) {
            ...
            $.ajax({
                type: "POST",
                url: SiteName + "/Api/HeaderMostPop",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    var msg = data
                    setItemF("objTMMostPop", data, 12)
                    buildMostPop(data);
                },
                error: function (xhr, desc, exceptionobj) {
...
function buildMostPop(obj) {
    ...
    $.each(obj, function () {
        ...
        switch (pack.deptNA) {
            case "ED":
                siteRoot = "www.tripmasters.com/europe";
                break;
            case "LD":
                siteRoot = "www.tripmasters.com/latin";
                break;
            case "TMAS":
                siteRoot = "www.tripmasters.com/asia";
                break;
        }
        relTxt = relTxt + '<li class="row liMostPop">';
        relTxt = relTxt + '<div>';
        relTxt = relTxt + '<a href="https://' + siteRoot + '/' + pack.CountryNA.replace(/\s/g, '_').toLowerCase() + '/' + pack.PDL_Title.replace(/\s/g, '_').toLowerCase() + '/package-' + pack.PDLID + '">';
