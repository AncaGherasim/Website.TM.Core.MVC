...
var myDate = new Date();
var jscal = 'S';
var backCookie = getItemF("localStorageTMBYO");
console.log(backCookie);
...
/*    if (cookVal != null) {
    ...
    /* *** With Air default on BYO *** */
    backCookie !== null ? cookieCheck() : loadAir();
});

function loadAir() {
    dateByDest();
    doitArr();
    $('.transportation__info').attr('style', 'display: flex')
    getHomeTownAirport();
}
function updateFlight(input) {
    ...
    if (value === 'False') {
        ...
        $('.transportation__info').attr('style', 'display: none')
        $('#qWair').val('False');
    } else if (value === 'True') {
        $('#AirParam').slideDown();
...
function addCityResponsive(fr, to, fcook) {
    if (to > 12) {
        ...
        var nwDiv = '<!-- city  ' + to + ' -->\
			<div class="main-calendar__add-city" id="dvCity' + to + '">\
			<div>\
			<label>\
            <span class="material-icons material-symbols-outlined">location_on</span>\
            <input name="qNACity'+ to + '" type="text" id="qNACity' + to + '" value="" placeholder=" "/>\
            ...
            <select class="main-calendar__appearance" name="qSTCity'+ to + '" id="qSTCity' + to + '">\
            ...
            <div >\
			<button id="spRemove' + to + '" onClick="qDelCty(' + to + ')" aria-label="Remove city"><span>✖</span><span>Remove</span></button>\
			</div>\
            </div>\
			</div><!-- end city '+ to + ' -->';
...
function byoValidation() {
    ...
    if (($('#qLeaveID').val() === '-1' || $('#qLeaveNA').val() == '') && $('#qWair').val() === "True") {
    ...
    $('form#formBYO input[id^="qIDCity"]').each(function () {
        if (this.value == '-1') {
            errorAlert(this.id.replace('ID', 'NA'), 'Select a valid city');
            cC -= 1;
        } 
        cC += 1;
...
function submitFormT4(frmNA) {
    $('.modal__wrapper > div:nth-child(2)').show()
    $('#transpCalendar').removeClass('is-hidden');
    ...
    setItemF("localStorageTMBYO", stringQuery, 1);
    var stringForm = '';
...
function buildTransportationFrom() {
    var cookSaved = JSON.parse(getItemF("localStorageTMBYO"));
    console.log("buildTransportationFrom cookSaved: " + cookSaved);
    $('#xaddFlight').val(cookSaved.qWair);
    $('#xLeaveCO').val(cookSaved.qLeaveNA);
    $('#xtxtLeavingFrom').val(cookSaved.qLeaveNA);
    $('#xtxtLeavingFromSpan').text(cookSaved.qLeaveNA);
    $('#xidLeavingFrom').val(cookSaved.qLeaveID);
    $('#xtxtBYArriving').val($('#qArrDate').val());
    $('#txtCabin').text($('#qCabinOpt option[value="' + cookSaved.qCabinOpt + '"]').text());
    $('#xCabin').val(cookSaved.qCabinOpt);
    $('#xgoingID').val(cookSaved.qgoingID);
    $('#arrivalDate').text($('#qArrDate').val());
...
function buildTransportationTo(jscity) {
    try {
        ...
        if (totCtys === undefined) {
            ...
            dvCities = dvCities + '<div id="cityTo' + dataCity['No'] + '" class="dvtranspCityList transportation__city">' +
                '<div>' +
                '<!-- City Name / ' + this.No + ' /-->' +
                ...
                '<div>';
            if (this.No === '1') {
                dvCities = dvCities + '<span>Arriving on</span>' + '<b>' + $('#qArrDate').val() + '</b>'
            }
            dvCities = dvCities + '</div>' +
                ...
                '<span>No of nights</span>' +
                '<span class="material-icons material-symbols-outlined" > schedule</span >' +
                '<select class="main-calendar__appearance" name="xSTCity' + dataCity['No'] + '" id="xSTCity' + dataCity['No'] + '">';
            for (i = 1; i <= 14; i++) {
                if (i == infocity[5]) { dvCities = dvCities + '<option value="' + i + '" selected>' + i + `${i === 1 ? ' night' : ' nights'}` + '</option>'; }
                else { dvCities = dvCities + '<option value="' + i + '">' + i + `${i === 1 ? ' night' : ' nights'}` + '</option>'; }
            };
            dvCities = dvCities + '</select>' +
                '<div class="main-calendar__arrow">›</div></label>' +
                '</div></div></div>';
            ...
            $.each(cityData, function (city) {
                ...
                if (isNaN(Number(this.No)) === true) {
                    if (this.No === 'S') {
                        ...
                        dvCities = dvCities + '<div id="cityTo' + this.No + '" class="dvtranspCityList transportation__cityS">' +
                            '<div>' +
                            '<!-- City Name / S /-->' +
                            '<div><span class="material-symbols-outlined material-icons">error</span> Must arrive to <b>' + this.PlaceName + '</b>' +
                        ...
                        if (this.Options !== null) {
                            ...
                            if (transpOpt.Ranking === undefined) {
                                $.each(transpOpt, function () {
                                    if (this.Ranking === 1) {
                                        ...
                                        dvTranChck = dvTranChck + '<label><input data-type="' + this.ProductType + '" data-field="' + this.ProductFreeField1 + '" data-ovrnts="' + this.Overnight + '" data-rank="' + this.Ranking + '" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" checked type="radio">' + this.ProductTypeName + '&nbsp;&nbsp;</label>'
                                        dvTranOpt = dvTranOpt + '<!-- Trasport option selected label -->' +
                                            '<p id="pTranspSel' + ctyNo + '-' + this.Ranking + '" class="' + selfCss + '">' +
                                            '<span><span>Travel by: </span>' + `<img src="https://pictures.tripmasters.com/siteassets/m/${this.ProductTypeName.replace(/\s/g, '_').toLowerCase()}_icon.gif" />` + '<span>' + this.ProductTypeName + '</span>';
                                        dvTranOpt = dvTranOpt + 'to ' + nxtCity;
                                        ...
                                        dvTranChck = dvTranChck + '<label><input data-type="' + this.ProductType + '" data-field="' + this.ProductFreeField1 + '" data-ovrnts="' + this.Overnight + '" data-rank="' + this.Ranking + '" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" type="radio">' + this.ProductTypeName + '&nbsp;&nbsp;</label>'
                                        dvTranOpt = dvTranOpt + '<!-- Trasport option selected label -->' +
                                            '<p id="pTranspSel' + ctyNo + '-' + this.Ranking + '" class="' + selfCss + '">' +
                                            '<span><span>Travel by: </span>' + `<img src="https://pictures.tripmasters.com/siteassets/m/${this.ProductTypeName.replace(/\s/g, '_').toLowerCase()}_icon.gif" />` + '<span>' + this.ProductTypeName + '</span>';
                                        dvTranOpt = dvTranOpt + 'to $$' + infocityS[1];
                                        dvTranOpt = dvTranOpt + '</span>' +
                                    ...
                                    if (this.ProductType === 'C') {
                                        ...
                                        dvTranOpt = dvTranOpt + '<!-- CAR options / parameters -->' +
                                            '<div class="transportation__car" id="carOptions' + ctyNo + '" ' + spDisplay + '>' +
                                            '<div id="pickup' + ctyNo + '">' +
                                            '<span>Pick-up</span>' +
                                            '<div>' +
                                            ...
                                            '</select>' +
                                            '<select name="xpickupDayCity' + ctyNo + '" id="xpickupDayCity' + ctyNo + '" class="transportation__no-border">' +
                                            '<option value="F">First Day</option>' +
                                            '<option value="L">Last Day</option>' +
                                            '</select>' +
                                            '</div>' +
                                            '</div>' +
                                            '<div id="trdropOff' + ctyNo + '">' +
                                            '<span>Drop-off</span>' +
                                            '<div>' +
                                            ...
                                            '<option value="F">First Day</option>' +
                                            '</select>' +
                                            '<select name="xdropoffCity' + ctyNo + '" id="xdropoffCity' + ctyNo + '" class="selectDropOffCity transportation__no-border" data-city="' + ctyNo + '">';
                                ...
                                if (transpOpt.Ranking === 1) {
                                    ...
                                    dvTranChck = dvTranChck + '<label><input data-type="' + transpOpt.ProductType + '" data-field="' + transpOpt.ProductFreeField1 + '" data-ovrnts="' + transpOpt.Overnight + '" data-rank="' + transpOpt.Ranking + '" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" checked type="radio">' + transpOpt.ProductTypeName + '&nbsp;&nbsp;</label>'
                                    dvTranOpt = dvTranOpt + '<!-- Trasport option selected label -->' +
                                        '<p id="pTranspSel' + ctyNo + '-' + transpOpt.Ranking + '" class="' + selfCss + '">' +
                                        '<span><span>Travel by: </span>' + `<img src="https://pictures.tripmasters.com/siteassets/m/${this.ProductTypeName.replace(/\s/g, '_').toLowerCase()}_icon.gif" />` + '<span>' + transpOpt.ProductTypeName + '</span>';
                                    dvTranOpt = dvTranOpt + 'to ' + nxtCity;
                                    dvTranOpt = dvTranOpt + '</span>' +
                                        '<span>' +
                                        '<input class="buttontranspChange" value="change" id="' + ctyNo + '" type="button">' +
                            ...
                            dvTranOpt = dvTranOpt + '<p id="pTranspSel' + ctyNo + '-' + Number(optCS + 1) + '" class="ptranspSelNoAct"><span><span>Travel by: </span>' + '<img src="https://pictures.tripmasters.com/siteassets/m/self_icon.gif" />' + '<span>On your own</span> to ' + nxtCity + '</span><span><input class="buttontranspChange" value="change" id="' + ctyNo + '" type="button"></span> </p>'
                            dvTranChck = dvTranChck + '<label><input data-type="W" data-field="OWN" data-ovrnts="0" data-rank="' + Number(optCS + 1) + '" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" type="radio">On your own</label>'
                            dvCities = dvCities + dvTranChck + '</p>' + dvTranOpt
                        }; // if option != undefined
                        dvCities = dvCities + '</div></div>'
                    };
                    if (this.No === 'E') {
                        ...
                        dvCities = dvCities + '<div id="cityTo' + this.No + '" class="dvtranspCityList transportation__cityS">' +
                            '<div>' +
                            '<!-- City Name / E /-->' +
                            '<div><span class="material-symbols-outlined material-icons">error</span> Must depart from <b>' + this.PlaceName + '</b>' +
                            '<input type="hidden" id="xNOCity' + this.No + '" name="xNOCity' + this.No + '" value="' + this.No + '" />' +
                            '<input name="xNACity' + this.No + '" type="hidden" id="xNACity' + this.No + '" value="' + this.PlaceName + '"/>' +
                            '<input type="hidden" name="xIDCity' + this.No + '"  id="xIDCity' + this.No + '" value="' + this.PlaceID + '"  />' +
                    ...
                    dvCities = dvCities + '<div id="cityTo' + this.No + '" class="dvtranspCityList transportation__city">' +
                        '<div>' +
                        '<!-- City Name / ' + this.No + ' /-->' +
                        '<div><span>' + decodeURIComponent(this.PlaceName) + '</span>' +
                        '<input type="hidden" id="xNOCity' + this.No + '" name="xNOCity' + this.No + '" value="' + this.No + '" />' +
                    ...
                    dvCities = dvCities + '</div>' +
                        ...
                        '<span>No of nights</span>' +
                        '<span class="material-icons material-symbols-outlined" > schedule</span >' +
                        '<select name="xSTCity' + this.No + '" id="xSTCity' + this.No + '" class="main-calendar__appearance">';
                    for (i = 1; i <= 14; i++) {
                        if (i == infocity[5]) { dvCities = dvCities + '<option value="' + i + '" selected>' + i + `${i === 1 ? ' night' : ' nights'}` + '</option>'; }
                        else { dvCities = dvCities + '<option value="' + i + '">' + i + `${i === 1 ? ' night' : ' nights'}` + '</option>'; }
                    };
                    ...
                    if (this.Options !== null) {
                        ...
                        if (this.Options[0].Ranking !== undefined) {
                            ...
                            dvTranOpt = dvTranOpt + '<p id="pTranspSel' + ctyNo + '-' + Number(optC + 1) + '" class="ptranspSelNoAct"><span><span>Travel by: </span>' + '<img src="https://pictures.tripmasters.com/siteassets/m/self_icon.gif" />' + '<span>On your own</span> to ' + nxtCity + '</span><span><input class="buttontranspChange" value="change" id="' + ctyNo + '" type="button"></span></p>'

                            dvTranChck = dvTranChck + '<label><input data-type="W" data-field="OWN" data-ovrnts="0" data-rank="' + Number(optC + 1) + '" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" type="radio">On your own</label>'
                        ...
                        if (this.No < totCtys && nxtCity != '') {
                            ...
                            dvTranChck = dvTranChck + '<input type="hidden" id="xOVNCity' + ctyNo + '" name="xOVNCity' + ctyNo + '" value="0"/>' +
                                '<p class="ptranspCheck" id="pCheckBox' + ctyNo + '">' +
                                '<label><input data-type="OWN" data-field="W" data-ovrnts="0" data-rank="1" id="xradioTrans' + ctyNo + '" name="xradioTrans' + ctyNo + '" class="checkBox" value="' + ctyNo + '" type="radio">On your own</label>'

                            var dvTranOpt = '<!-- Trasport option selected label -->' +
                                '<p id="pTranspSel' + ctyNo + '-1" class="' + selfCss + '">' +
                                '<span><span>Travel by: </span>' + '<img src="https://pictures.tripmasters.com/siteassets/m/self_icon.gif" />' + '<span>On your own</span> to ' + nxtCity +
                                '</span>' +
                                '<span>' +
                                '<input class="buttontranspChange" value="change" id="' + ctyNo + '" type="button">' +
                                '</span>' +
        ...
        $('#dvWaitGlobe').hide();
        $('.transportation__wrapper').show();
        $('.transportation__button-wrapper').css('display', 'flex');
        $('#dvTranspError').hide();
        $(dvCities).insertAfter('.dvtranspLabels');
        $(dvCities).insertAfter('#transLabels');
        ...
        $('.goToStartAgain, #btnEdit, #btnBack, #btnBackDsk').click(function () { startAgain(this); });
        dvCities = '';
        checkIfCarIsFirst();
    }
    catch (err) {
        buildTransportationError(err.message);
        logErrorToCloudWatch(err.message, jscity)
            .then(() => {
                console.log('Error logged successfully to CloudWatch');
            })
            .catch((apiError) => {
                console.error('Failed to log error to CloudWatch:', apiError);
            });
    }
}
function logErrorToCloudWatch(errorMessage, jscity) {
    jscity = (jscity == undefined) ? "undefined" : ((jscity == null) ? "null" : ((jscity == []) ? "null array" : jscity.toString()));
    return fetch('/Api/errorcalendarlogs', {
        method: 'POST',
        headers: {
    ...
    .then((response) => {
        if (!response.ok) {
            throw new Error(`Failed to log error. Status: ${response.status}, Message: ${response.statusText}`);
        }
...
function buildTransportationError(err) {
    $('.goToStartAgain, #btnEdit, #btnBack, #btnBackDsk').click(function () { startAgain(this) });
    var cookieValues = JSON.parse(getItemF("localStorageTMBYO"));
    var cokC = 0;
...
function buildFromCook(build) {
    ...
    if (build === 1) {
        $.each(cookieValues, function (i, e) {
            i === 'qWair' ?
                e === 'True' ? ($('#qLeaveNA').val(cookieValues.qLeaveNA), $('#updateFlightToTrue').trigger('click')) : ($('#updateFlightToFalse').trigger('click')) : '';
...
function openRoom(rm, no) {
    no === 0 ? ($('.transportation__rooms-settings, #pPaxLabels').slideUp('fast')) : ($('.transportation__rooms-settings, #pPaxLabels').slideDown('fast'));
...
function changeTranspOption(obj) {
    ...
    $('#pCheckBox' + obj.id + '').show().attr('style', 'display: flex;');
};
...
function startAgain(obj) {
    $('#transpCalendar').addClass('is-hidden');
    $('body').removeClass('modal-open');
    $('.transportation__wrapper').hide();
    $('.transportation__button-wrapper').hide();
    $('div.dvtranspCityList').remove();
    $('#dvTranspError').hide();
    $('#xcabinRoomPax').val('1 Room, 2 Travelers, Economy');
    $('#xiRoomsAndPax').val('1|2');
    selectRoomPax('1|2');
    $("select[id^='xiAdults']").val('2')
    $("select[id^='xiChild']").val('-1')
    $("select[id^='xRoom2_iAdults']").val('0')
    $("select[id^='xRoom2_iChild']").val('0')
    $("select[id^='xRoom3_iAdults']").val('0')
    $("select[id^='xRoom3_iChild']").val('0')
}
...
function submitToBP(strQ, jsR) {
    ...
    if (jsRprts[1] === '1') {
        $subForm.find('input').each(function () {
            ...
            /iChildren/.test(this.id) ? !/Room/.test(this.id) ? (this.value = strQ.xiChildren, $('#xiChildren').val('0')) : '' : '';
            ...
            /Room2_iAdults/.test(this.id) ? strQ.xRoom2_iAdults ? (this.value = strQ.xRoom2_iAdults, $('#xRoom2_iAdults').val('0')) : this.value = '' : '';
            /Room2_iChildren/.test(this.id) ? strQ.xRoom2_iChildren ? (this.value = strQ.xRoom2_iChildren, $('#xRoom2_iChildren').val('0')) : this.value = '' : '';
            ...
            /Room3_iAdults/.test(this.id) ? strQ.xRoom3_iAdults ? (this.value = strQ.xRoom3_iAdults, $('#xRoom3_iAdults').val('0')) : this.value = '' : '';
            /Room3_iChildren/.test(this.id) ? strQ.xRoom3_iChildren ? (this.value = strQ.xRoom3_iChildren, $('#xRoom3_iChildren').val('0')) : this.value = '' : '';
...
function resetForm() {
    ...
    frmToBook.reset();
    //frmToBook = $(frmToBook).filter(function (item) { return item.name !== "__RequestVerificationToken" })
    $(frmToBook).find('input').each(function () {
        if (this.id === 'GeneralLoc' || this.id === "bFirst" || this.id === "sortFlag" || this.id === "MinStay" || this.id === "addCar" || this.id === "iOutTime" || this.id === "iInTime" || this.id === "addTransfer_IC" || this.id === "") {
            //do nothing
            //console.log(this.id);
        } else {
            $('#' + this.id).val('');
...
function childAge(tag) {
    ...
    if (totpax > 6) {
        ...
        $('#pChildAges').show();
        $('#pChildAges' + rnum + '').show().attr('style', 'display: flex;').find('select').each(function () { this.id.slice(-1).match(isNumber) <= Number(oval) ? ($(this).show(), $('label[for="' + this.id + '"]').show()) : ($(this).hide().val(-1), $('label[for="' + this.id + '"]').hide('fast', function () { countChilds() <= 0 ? $('#pChildAges').hide() : '' })) });
    };
};
