...
$(document).ready(function() {
	...
	$dvMAsk=$('#mask');
		...
		$('#qq').click(function (e) {
			...
			var popW = popL[0] - 200
			var popH = popL[1]
			$dvSearch.attr('style','padding:20px; position:absolute; width:480px; height: 110px; display:none; z-index:9999; left:'+ popW + 'px; top:'+ popH +'px'); 
...
// --- TO GET RELATED PACKS FOR INFO DIV --- ///
function relPackCallXXX(packID,site,plc){
	var relTxt = '';
	var siteLink = 'europe';
	var divfill; 
	plc != undefined ? divfill = 'dvRelPack'+packID+plc : divfill = 'dvRelPack'+packID
	$.ajax({
		type: "POST",
		url: "/europe/WS_Library.asmx/getDataRelPacks",
		contentType: "application/json; charset=utf-8",
		dataType: "json",
		data: '{packID:"' + packID + '", site:"'+ site +'"}',
		success: function(data) {
			...
			if (msg != ''){
				...
				for(i=0;i<=strPrts.length -1;i++){
					 ...
					 relTxt = relTxt + '<a href="http://www.tripmasters.com/'+siteLink+'/'+echP[0]+'_Vacations.aspx" style="margin-right:10px"><u>'+echP[0]+' ('+echP[1]+')</u></a>';
				};
				$('#'+divfill+'').html(relTxt).slideDown('slow');
			};
		},
           ...
           // console.log("Success: " + data);
            ...
            if (msg != '') {
                ...
                for (i = 0; i <= strPrts.length - 1; i++) {
                    ...
                    relTxt = relTxt + '<a href="/europe/' + echP[0].replace(/\s/g, '_') + '_Vacations.aspx" style="margin-right:10px">'
...
function windowCMS(id,w,h){
	//'/cms/<%=innLn(3)%>/Web_Content.aspx?cms&wh=0&wf=0
	var LeftPosition; 
	var TopPosition; 
	var settings;
	var navAg = navigator.userAgent;
	var dom = document.domain;
	var newURL = '/europe/cms/' + id + '/Web_Content.aspx?cms&wh=0&wf=0'; //tourl.replace('latindestinations.com',dom); //tourl
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (msg != ''){
				...
				$('#dvInfoContainer').css({'top':''+ Number(objPos.top)-200 +'px','z-index':'8110','position':'absolute'}); 
