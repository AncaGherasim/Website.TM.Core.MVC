...
namespace MVC_TM.Infrastructure
{
    public class CachedDataService
    {
        ...
        public async Task LoadDestinationsIfNecessary() //IfNecessary means if the property is empty or expired
        {
        ...
        private async Task LoadFeedbacksFromDatabase()
        {
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    Console.WriteLine("****** CachedDataService - TM - Read feedbacksCache from DataBase, memorysize is " + System.Text.Encoding.UTF8.GetByteCount(Newtonsoft.Json.JsonConvert.SerializeObject(feedbacksCache)));
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("****** CachedDataService - TM - Error reading feedbacksCache from DataBase");
        ...
        private async Task LoadDestinationsFromDatabase()
        {
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    Console.WriteLine("****** CachedDataService - TM - Read destinationsCache from DataBase, memorysize is " + System.Text.Encoding.UTF8.GetByteCount(destinationsCache));
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("****** CachedDataService - TM - Error reading destinationsCache from DataBase");
                Console.WriteLine(ex.Message + ex.StackTrace);
            }
        }
        ...
        private async Task LoadDepCitiesFromDatabase()
        {
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    Console.WriteLine("****** CachedDataService - TM - Read depCitiesCache from DataBase, memorysize is " + System.Text.Encoding.UTF8.GetByteCount(Newtonsoft.Json.JsonConvert.SerializeObject(depCitiesCache)));
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("****** CachedDataService - TM - Error reading depCitiesCache from DataBase");
                Console.WriteLine(ex.Message + ex.StackTrace);
            }
        }
        ...
        private async Task LoadPriorCitiesFromDatabase()
        {
            try
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    Console.WriteLine("****** CachedDataService - TM - Read priorCitiesCache from DataBase, memorysize is " + System.Text.Encoding.UTF8.GetByteCount(Newtonsoft.Json.JsonConvert.SerializeObject(priorCitiesCache)));
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("****** CachedDataService - TM - Error reading priorCitiesCache from DataBase");
                Console.WriteLine(ex.Message + ex.StackTrace);
            }
        }
        private async Task<string> LoadWebAnnouncement()
        {
            ...
            {
                Console.WriteLine("****** CachedDataService - TM - Error reading webannouncement from DataBase");
                Console.WriteLine(ex.Message + ex.StackTrace);
                return string.Empty;
            }
        }
