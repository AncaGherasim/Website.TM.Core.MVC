...
namespace MVC_TM.Infrastructure
{
    public class SqlCalls
    {
        ...
        {
            return "SELECT PRWT.PWT_TemplateValue  FROM PRD_ProductItemXwebTemplate PIXT INNER JOIN PRD_ProductWebTemplate PRWT ON PIXT.PPWT_TemplateID = PRWT.PWTID WHERE PIXT.PPWT_ProdItemID = " + PackageId +
                " AND PIXT.PPWT_Active = 1 AND PRWT.PWT_Active = 1 ORDER BY PWT_TemplateValue DESC";
        }

        public static string SQL_RelPackByPackID(string PackageId, string siteLike, string siteID)
        {
            return @"SELECT DISTINCT pxp.CXZID,pxp.cxz_productitem as PackageId,pit.PDL_title as PackageTitle,pxp.cxz_ChildPlaceId as PlaceId,ph.str_placetitle as PlaceTitle,
                ...
                INNER JOIN PRD_productItem pit2 ON pit2.pdlid = pxp2.cxz_productitem
                AND pit2.pdl_active = 1
                AND pit2.PDL_NoWeb = 0
                INNER JOIN PRD_Product pro2 ON pro2.spdid = pit2.pdl_productid
                AND pro2.spd_active = 1
                AND pro2.spd_producttypesyscode = 34
                AND PRO2.SPD_InternalComments LIKE '%" + siteLike + @"%'
                WHERE(pxp2.cxz_childplaceid = pxp.cxz_ChildPlaceId)) AS NoOfPacks, ph.str_placetypeid,PROD.SPD_Features
                FROM prd_placexproductitem pxp
                INNER JOIN str_places_hierarchy ph ON ph.str_placeid = pxp.cxz_childplaceid
                AND ph.str_userid = " + siteID + @" and ph.str_placetypeid in (1, 5, 6, 25)
                ...
                AND PROD.SPD_InternalComments LIKE '%" + siteLike + @"%'
                WHERE
                pxp.cxz_active = 1
                And ph.str_placeactive = 1
                and ph.STR_NoWeb = 0
                AND pxp.cxz_productitem in (" + PackageId + @") ORDER BY str_placetitle";
        }

        public static string SQL_PackageRelatedPlacesInfo_WithCMSs(string PackageId)
        {
            return @"declare @placesIds table(PlaceId int, PlaceTypeId int, INDEX IX1 CLUSTERED(PlaceId, PlaceTypeId))
                insert into @placesIds 
                select distinct pxp1.cxz_ChildPlaceId, ph.str_placetypeid
                FROM prd_placexproductitem pxp1                 
                INNER JOIN STR_Places_Hierarchy ph ON ph.str_placeid=pxp1.cxz_childplaceid AND ph.str_prodkindid=0 And ph.str_placeactive = 1 And ph.str_placetypeid <> 3 AND ph.STR_NoWeb = 0 AND ph.str_UserId in (243, 595, 182) 
                WHERE pxp1.cxz_active = 1 AND pxp1.cxz_productitem=" + PackageId + @"
                ;with Hotels(PDLID, GIPH_TNTournetName, GIPH_GIATAID, CXZ_ChildPlaceID, N, N1) as 
                      (select gpi.GLT_PDLID, gh.GIPH_TNTournetName, gh.GIPH_GIATAID, gxtp.GITP_PLCID, row_number() over(partition by gxtp.GITP_PLCID, gh.GIPH_GIATAID order by gh.GIPH_TNSequence, pIt.PDLID), row_number() over(partition by gxtp.GITP_PLCID, gh.GIPHID order by gh.GIPHID)
                       ...
                       inner join @placesIds ids on ids.PlaceId = gxtp.GITP_PLCID and ids.PlaceTypeId in (25,1)
                       where gxtp.GITP_Active = 1)
                ,CMSs(STRID, STR_PlaceID, CMSW_Title, CMSW_Order, CMSW_RelatedCmsID) as 
                       (SELECT plcH.STRID, plcH.STR_PlaceID,Xcms.CMSW_Title, Xcms.CMSW_Order, Xcms.CMSW_RelatedCmsID
                        FROM STR_WebHierarchyXCMS Xcms
                        INNER JOIN STR_Places_Hierarchy plcH ON STR_PlaceActive = 1 AND STR_UserID = 243 AND Xcms.CMSW_WebHierarchyID = plcH.STRID 

                        inner join @placesIds ids on ids.PlaceId = plcH.STR_PlaceID
                ...
                ,SSs(PlaceId, NoOfSS) as
                       (SELECT CXZ_ChildPlaceID, count(1) as NoOfSS
                        ...
                        inner join @placesIds ids on ids.PlaceId = CXZ_ChildPlaceID and ids.PlaceTypeId in (25, 1)
                ...
                ,Feeds(PlaceId, NoOfFeeds, OverAll) as
                       (Select ids.PlaceId, count(*), round(avg(convert(decimal(12, 2), case when[PCC_OverallScore] = 0 then null else[PCC_OverallScore] end)),1)
                        ...
                        Where ids.PlaceTypeId = 5 group by ids.PlaceId)
                Select pxp.cxz_ChildPlaceId as PlaceId, ph.str_placetypeid, ph.Str_PlaceTitle, ph.Str_PlaceTypeId, Packs.NoOfPacks, isnull(cF.NoOfFeeds, 0) as NoOfFeeds, isnull(cF.OverAll, 0) as OverAll
                      ,isnull(HotelsXPlace.NoOfHotels, 0) as NoOfHotels, isnull(SSs.NoOfSS, 0) as NoOfSS
                      ,case when row_number() over(Partition by ph.str_placeid Order by ph.str_placeid, str_PlacePriority) = 1 then isnull(ph.STR_PlaceShortInfo, '') else '' end as STR_PlaceShortInfo
                      ,row_number() over(Partition by ph.str_placeid Order by ph.str_placeid, str_PlacePriority) as N, CMSs.CMSW_Title, CMSs.CMSW_Order, CMSs.CMSW_RelatedCmsID
               FROM prd_placexproductitem pxp
                INNER JOIN STR_Places_Hierarchy ph ON ph.str_placeid = pxp.cxz_childplaceid AND ph.str_prodkindid = 0 And ph.str_placeactive = 1 And ph.str_placetypeid <> 3 AND ph.STR_NoWeb = 0 AND ph.str_UserId in (243, 595, 182)
                ...
                left join CMSs on CMSs.STRID = ph.strid
                ...
                outer apply(SELECT COUNT(PdlId) as NoOfPacks FROM PRD_Placexproductitem pxp2
                         INNER JOIN PRD_productItem pri ON pri.pdlid = pxp2.cxz_productitem and pri.PDL_NoWeb = 0 and pri.PDL_Active = 1 and pri.PDL_Title NOT LIKE 'Zpend%' and pri.PDL_Title NOT LIKE 'zzz%' and pri.PDL_IsPackage = 1
                          INNER JOIN PRD_Product pro2 ON pro2.spdid = pri.pdl_productid AND pro2.spd_active = 1 AND pro2.SPD_InternalComments LIKE '%" + InternalComments + @"%' AND pro2.SPD_ProductTypeSysCode = 34 and pro2.SPD_StarratingSysCode <> 541
                          WHERE pxp2.cxz_childplaceid = pxp.cxz_ChildPlaceId and pxp2.cxz_active = 1) Packs
                  WHERE pxp.cxz_active = 1 AND pxp.cxz_productitem = " + PackageId;
        }
        ...
        public static string SQL_PackageRelatedItineraries(string PackageID)
        {
        ...
        public static string SQL_PackageCMS(string PackageID)
        {
            return @"Select isNull(PICMS_ItineraryCMSID, 0) as PICMS_ItineraryCMSID
               ...
               ,isNull(PICMS_OverviewCMSID, 0) as PICMS_OverviewCMSID
                From PRD_ProductItemXCMSs Where PICMS_Active = 1 AND PICMS_ProductItemID = " + PackageID;
        }
        ...
        public static string SQL_PackageComponentList(string PackageID)
        {
            return @"SELECT DISTINCT PRIx.PDLID as ParentID, PRIx.PDL_Title as ParentTitle, PROx.SPD_StatePlaceID as City, PRIx.PDL_Gateway as GateWay, PRIx.PDL_ReturnCity as ReturnCity, case when (isnull(PRIx.PDL_PercentMarkupOverride,0) + isnull(PRIx.PDL_FlatMarkupOverride,0))>0 then 1 else 0 end as MarkUpByPkg 
                  ...
                  ,P.PLC_Title as CityTitle, P.PLCID as CityID, 1 as cmp_CategoryFlag, 'No' as cmp_OptionalFlag 
                  ,cast(isnull(cmps.value('count(/RelatedID)', 'int'),'0') as varchar(1000)) as RelatedProductItems, cast(isnull(cmps.query('/RelatedID'),'') as varchar(max)) as RelatedIDs 
                  ,cast(isnull(api.query('for $id in /Id return data($id)'),'') as varchar(1000)) as Pkg_AirVendorAPI, cast(category as varchar(max)) as category 
        ...
        {
            return "Select dbo.GetHotelBlackoutsList(" + PackageID + ") as blacklist";
        }
        ...
        {
            return @"declare @page int = " + page + ", @tabType varchar(3); set @tabType = '" + tabType + "' if @tabType = 'C' with FeedsC(PCCID, PCC_Comment, PCC_CustomerName, PCC_Itinerary, OverallScore, N) as (Select CF.PCCID, CF.PCC_Comment, CF.PCC_CustomerName, CF.PCC_Itinerary, CF.PCC_OverallScore, row_number() over(Order by PCCID desc) as N From STR_Places_Hierarchy PLCO inner join PRD_PlaceXProductItem pXp on pXp.CXZ_ChildPlaceID = PLCO.STR_PlaceID and pXp.CXZ_Active = 1 inner join PRD_ProductItem pri ON pXp.CXZ_ProductItem = pri.PDLID and pri.PDL_NoWeb = 0 and pri.PDL_Active = 1 inner join PRD_CustomerComment CF on pri.PDLID = CF.PCC_PDLID and datalength(CF.PCC_Comment) > 15 and CF.PCC_Active = 1 and CF.PCC_Block = 0 Where PLCO.STR_PlaceID = " + id + " and PLCO.STR_UserID = " + SiteUserId + " and PLCO.STR_NoWeb = 0 and PLCO.STR_PlaceActive = 1 and PLCO.STR_PlaceTypeID = 5 and PLCO.STR_ProdKindID = 0) select* from FeedsC where n between((@page-1)*10)+1 and @page*10 order by PCCID desc else with FeedsP(PCCID, PCC_Comment, PCC_CustomerName, PCC_Itinerary, OverallScore, N) as (SELECT CF.PCCID, CF.PCC_Comment, CF.PCC_CustomerName, CF.PCC_Itinerary, CF.PCC_OverallScore, row_number() over(Order by PCCID desc) FROM PRD_CustomerComment CF  WHERE CF.PCC_PDLID = " + id + " AND datalength(CF.PCC_Comment) > 15 AND CF.PCC_Active = 1 AND CF.PCC_Block = 0) select* from FeedsP where n between((@page-1)*10)+1 and @page*10";
        }
        ...
        public static string SQL_GetCustomerCommentsByuserID(string packids)
        {
            return @"SELECT top 3 PRI.PDLID,PRI.PDL_Title, Convert(money, isnull(STP.STP_Save, 99999)) as STP_Save 
                     ,CASE WHEN STP.STP_NumOfNights is null then PRI.PDL_Duration ELSE STP.STP_NumOfNights END as STP_NumOfNights 
                     ,HRCH.STR_PlaceTitle as CityNA, PRO.SPD_CountryPlaceID, CONT.STR_PlaceTitle as CountryName, img.IMG_Path_URL 
                     ...
                     WHERE CF.PCC_PDLID = PRI.PDLID AND CF.PCC_Comment is not null AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 15 AND CF.PCC_Active = 1 AND CF.PCC_Block = 0 AND CHARINDEX('---', CF.PCC_Comment) = 0 AND CFH.dep_date > convert(Varchar(10),Getdate()- 730,101) ORDER BY CF.PCC_Ranking ASC, CFH.dep_date DESC) as Comment 
                     ,(SELECT top 1 CFH.dep_date FROM PRD_CustomerComment CF INNER JOIN RSV_Heading CFH with (nolock) ON CF.PCC_BookingID = CFH.ID 
                     WHERE CF.PCC_PDLID = PRI.PDLID AND CF.PCC_Comment is not null AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 15 AND CF.PCC_Active = 1 AND CF.PCC_Block = 0 AND CHARINDEX('---', CF.PCC_Comment) = 0 AND CFH.dep_date > convert(Varchar(10),Getdate()- 730,101) ORDER BY CF.PCC_Ranking ASC, CFH.dep_date DESC) as dep_date
                     FROM STR_SitePromotion STP 
                     INNER JOIN PRD_ProductItem PRI ON PRI.PDLID = STP.STP_ProdItemID 
                     INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID 
        ...
        public static string SQL_SpotLights_Home(string plcIDs)
        {
            return @"SELECT PRI.PDLID, CONT.STR_PlaceTitle as CountryNA, CONT.STR_UserID, 
                CASE 
                When CONT.STR_UserID = 595 then 'TMAS' 
                When CONT.STR_UserID = 182 then 'LD' 
                When CONT.STR_UserID = 243 then 'ED' 
                else '' end as deptNA,
                ...
                ,IMG.IMG_Path_URL,IMG.IMG_500Path_URL, SPD_Description, PRI.PDL_Content as Included
                FROM PRD_ProductItem PRI 
                ...
                AND STP.STP_UserID in (243, 595, 182) AND STP.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101)
                AND STP.STP_EndDate >= CONVERT(VARCHAR(10), GETDATE(), 101)
        ...
        public static string SQL_AllDestinos(bool First3 = false)
        {
            string strQuery = @"SELECT p1.STR_PlaceID as CtyID, p1.STR_PlaceTitle as CtyNA, p2.STR_PlaceID as CouID, p2.STR_PlaceTitle as CouNA, p1.STR_PlaceAIID as Ranking, p1.STR_UserID as RegionID
                FROM  STR_Places_Hierarchy p1
                INNER JOIN STR_Places_Hierarchy p2 ON (p2.STR_PlaceID = p1.STR_Place1ParentID OR p2.STR_PlaceID = p1.STR_Place2ParentID)
                WHERE p1.STR_PlaceActive = 1
        ...
        public static string SQL_HotelInfo(string hotID)
        {
            return @"select pri.PDLID, gh.GIPH_TNTournetRating as Rating, GIPH_TNTournetName as Name, gh.GIPH_Name as GiataName, gh.GIPH_TNSequence as SequenceNo 
            ...
            where pri.PDLID = " + hotID + @" and pri.PDL_Active = 1 and pri.PDL_NoWeb = 0 AND pri.PDL_Title NOT Like '-%' AND pri.PDL_Title NOT Like 'Zblock%'";
        }
        ...
        public static string SQL_HotelFacilities(string hotID)
        {
            return @"declare @Source varchar(10)
               ...
               select @Source = case when gh.GIPH_TNContentSource like '%tournet%' then 'TN' else 'GIATA' end ,@giatahid= gh.GIPHID, @UseTournet = gh.GIPH_TNUseTournetContent from GIATA_GiataXProductItem gpi inner join GIATA_Hotels gh on gpi.GLT_GIPHID = gh.GIPHID and gh.GIPH_Active=1 and  gh.GIPH_TNNoWeb = 0  
                 where gpi.GLT_PDLID in (" + hotID + @") and gpi.GLT_Active=1 
               ...
               select f.GHGFID as Id, f.GHGF_SectionType as FacilityType, a.GHGA_SectionTitle as FacilityStrType, a.GHGA_FactTitle as FacilityName 
                ...
                where gpi.GLT_PDLID in (" + hotID + @") And gpi.GLT_Active = 1 order by f.GHGF_SectionType,  a.GHGA_SectionTitle";
        }

        public static string SQL_HotelRoomCategories(string hotID)
        {
            return @"Select PTYID, PTY_Title, isNull(PTY_Description,'none') as PTY_Description
                ...
                FROM PRD_Category WHERE PTY_ProductItemID = (" + hotID + @") AND PTY_Active = 1 ORDER BY PTY_Title";
        }
        ...
        public static string SQL_Hotels_From_Place(string plcID)
        {
          ...
          , isnull(GIPH_TNHighlights,''),gh.GIPH_TNSequence, isnull(ghs.GHS_TrustYouScore, 0),isnull(ghs.GHS_SolarToursScore, 0)
           ...
           , p.GIPH_TNHighlights
                 ...
                 , isnull(img.IMG_Path_URL, '') as IMG_Path_URL
        ...
        public static string SQL_PlacePackagesIdeas(string placeID)
        {
            return @"SELECT PRI.PDLID,
                PRI.PDL_Title,
                PRI.PDL_Duration,
                PHI.STR_PlaceTitle,
                CONVERT(VARCHAR(10), SPR.STP_EndDate, 101) as STP_EndDate,
                cast(isnull(SPR.STP_Save,9999) as money) STP_save,
                cast(isnull(SPR.STP_Price,9999) as money) STP_Price,
                SPR.STP_Minititle,
                SPR.STP_NumOfNights,
                PRI.PDL_StarRatingSysCode,
                PRI.PDL_Places,
                PRI.PDL_SequenceNo,
                PHI.STR_PlaceID,
                PHI.STR_PlaceTypeID,
                IMG.IMG_Path_URL,
                IMG.IMG_Title,
                SPR.STP_TypePromotion,
                PRI.PDL_Places,
                PRI.PDL_CustomerRanking,
                PPW.SPPW_Weight,
                isnull(PLCO.STR_PlaceTitle,'') as CountryName
                FROM STR_Places_Hierarchy PHI
                INNER Join PRD_PlaceXProductItem PXP ON PXP.CXZ_ChildPlaceID = PHI.STR_PlaceID
                ...
                LEFT JOIN PRD_ProductXImages PXI ON PXI.PXI_ProductID = PRI.PDL_ProductID AND PXI.PXI_Active = 1 AND PXI.PXI_Sequence = 0
                LEFT JOIN APP_Images IMG ON IMG.IMGID = PXI.PXI_ImageID 
                LEFT JOIN STR_PlacesXPackageWeight PPW
                ON PPW.SPPW_parentPlace = PHI.STRID AND PPW.SPPW_PackageID = PRI.PDLID AND PPW.SPPW_Active = 1 AND PPW.SPPW_MasterContentID = 0
                ...
                WHERE PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 AND IMG.IMG_Active = 1 
                AND (PRO.SPD_Active = 1) AND (PRO.SPD_ProductTypeSysCode = 34) AND (PRO.SPD_InternalComments LIKE '%" + InternalComments + @"%') 
                AND (PHI.STR_PlaceActive = 1) AND (PHI.STR_NoWeb = 0) AND PHI.STR_UserID = " + SiteUserId + @"
                AND (PHI.STR_PlaceID = " + placeID + @")
                ORDER BY PDL_SequenceNo,STR_PlaceTitle,STP_Price ASC,PDL_Title";
        }

        public static string SQL_Vacations_Places_Hierarchy(string PlaceTitle, bool IsInList = false)
        {
            return @"SELECT STRID,STR_PlaceID,STR_PlaceTitle,STR_PlaceTypeID,STR_PlaceShortInfo,STR_PlaceAIID,STR_PlaceInfo,STR_ProdKindID,isNull(STR_PageTemplate,'T1') as STR_PageTemplate
        ...
        public static string SQL_ManagerDisplayArea(Int32 AreaID)
        {
            return @"Select SDP_DisplayTitle
                  ...
                  Where SDP_UserID =  243 AND SDP_MasterContentID = 0 AND SDP_PlaceID =  " + AreaID + @"  
                  AND SDP_Active = 1
        ...
        {
            return @"with Cities (STR_PlaceTitle, STR_PlaceID, STR_PlaceShortInfo, STR_PlaceTypeID, SPW_Weight, STR_PlaceAIID, Country, CountryId)
                as (Select plh.STR_PlaceTitle, plh.STR_PlaceID, plh.STR_PlaceShortInfo, plh.STR_PlaceTypeID, SPW_Weight, plh.STR_PlaceAIID
                ...
                select *
                FROM Cities 
                WHERE STR_PlaceID in (
                        SELECT PHI.STR_PlaceID
                        FROM PRD_PlaceXProductItem PXP 
                        ...
                        LEFT Join STR_SitePromotion SPR ON SPR.STP_ProdItemID =PRI.PDLID AND SPR.STP_Active = 1 AND SPR.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101) 
                             AND SPR.STP_EndDate >= CONVERT(VARCHAR(10), GETDATE(), 101) AND SPR.STP_UserID = 243
        ...
        public static string SQL_CountriesWCommentsByDeptID()
        {
            return @"Select DISTINCT PLCO.STR_PlaceTitle as CountryName
                 ...
                 AND CFH.dept in (868, 1615)
                 AND CF.PCC_Active = 1 AND CF.PCC_Block = 0
                 AND CFH.dep_date > convert(Varchar(10), Getdate() - 730, 101)
                 AND CF.PCC_Comment is not null
        ...
        {
            return @"SELECT TOP 50 CF.PCC_PDLID,CF.PCC_Comment,CF.PCC_CustomerName,CF.PCC_Itinerary,CF.PCCID,CFH.dep_date, CFP.PDL_Title,PLCO.STR_PlaceTitle as CountryName,PLCO.STR_PlaceID as CountryID 
          FROM PRD_CustomerComment CF 
          INNER JOIN RSV_Heading CFH with (nolock) ON  CF.PCC_BookingID = CFH.ID
          INNER JOIN PRD_ProductItem CFP ON  CFP.PDLID = CF.PCC_PDLID
          INNER JOIN PRD_Product PRO ON PRO.SPDID = CFP.PDL_ProductID 
          ...
          AND CFH.dep_date > convert(Varchar(10),Getdate()-720,101) 
          AND CF.PCC_Comment is not null AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 15
          AND PLCO.STR_PlaceID in (" + CoutryIDs + @")  AND CHARINDEX('---', CF.PCC_Comment) = 0
          ORDER BY CF.PCC_Ranking ASC, CFH.dep_date DESC";
        }
        ...
        {
            return @"Select PRI.PDLID,PHY.STR_USerID
                ,PRI.PDL_Title
                ,cast(isnull(PRM.STP_Save,9999) as money) as STP_Save,Pic3.IMG_Path_URL,Pic3.IMG_500Path_URL
                ,isnull((Select top 1 isnull(ltrim(rtrim(PLC.PLC_Title)) ,'none') +' for '+ isnull(convert(varchar(5)
                ,STP_NumOfNights),0) + ' nights' as PLC_Title
                From PRD_Place PLC
                WHERE(PLC.PLCID = PRM.STP_FromPlaceID)
                AND PLC.PLC_Active = 1 
                AND PLC.PLC_PlaceTypeID in (1)),'none') as fromPlace
                ,PRI.PDL_Content 
                ,(SELECT Count(CF.PCCID) as feedbacks
                FROM PRD_CustomerComment CF 
                WHERE(CF.PCC_PDLID = PRI.PDLID)
                AND CF.PCC_Active = 1 and CF.PCC_DetailID = 0 AND datalength(CF.PCC_Comment) > 15
                AND CF.PCC_Block = 0) as feedbacks
                ,PHY.STR_PlaceTitle
                FROM PRD_ProductItem PRI
                INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID 
                LEFT JOIN STR_SitePromotion PRM ON PRM.STP_ProdItemID = PRI.PDLID 
                AND PRM.STP_UserID in  (243,595,182) 
                AND PRM.STP_Active = 1 
                AND PRM.STP_StartDate <= getdate() 
                AND PRM.STP_EndDate >= getdate()
                LEFT JOIN PRD_ProductXImages Pic2 ON Pic2.PXI_ProductID = PRI.PDL_ProductID 
                AND Pic2.PXI_Sequence  = 0 
                AND Pic2.PXI_Active =1
                LEFT JOIN APP_Images Pic3 ON Pic3.IMGID = Pic2.PXI_ImageID
                LEFT JOIN STR_Places_Hierarchy PHY ON PHY.STR_PlaceID = PRO.SPD_CountryPlaceID
                AND PHY.STR_PlaceTypeID = 5
                AND PHY.STR_PlaceActive = 1
                AND PHY.STR_ProdKindID = 0
                AND PHY.STR_USerID in  (243,595,182) 
                AND PHY.STR_PlacePriority = 1 
                WHERE PRI.PDLID in (" + VisitedPackagesIds + @") 
                AND PRI.PDL_Active = 1 
                AND PRI.PDL_NoWeb = 0
                AND PRO.SPD_ProductTypeSysCode = 34 
        ...
        {
            return @"Select PRI.PDLID, PRI.PDL_Title, PRI.PDL_SequenceNo, STP.STP_UserID 
               ,isnull(try_Convert(money,STP.STP_Price),99999) as STP_Price, isnull(try_Convert(money,STP.STP_Save),99999) as STP_Save 
               ,case when STP.STP_NumOfNights is null then PRI.PDL_Duration else STP.STP_NumOfNights end as STP_NumOfNights 
               ,PRO.SPD_CountryPlaceID as CountryID 
               ,(Select top 1 PLCO.STR_PlaceTitle FROM STR_Places_Hierarchy PLCO where PLCO.STR_PlaceID = PRO.SPD_CountryPlaceID and PLCO.STR_PlaceActive = 1 AND PLCO.STR_NoWeb = 0) as CountryName 
               FROM STR_SitePromotion STP 
               ...
               INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID AND PRO.SPD_Active = 1 
               WHERE STP.STP_UserID IN (182,243,595) AND STP.STP_Active = 1 and  STP_TypePromotion = 1 
               ...
               ORDER BY case STP_UserID when 243 then 1 when 595 then 2 when 182 then 3 end, Case When STP_TypePromotion = 0 Then 99 else STP_TypePromotion End ASC, STP_StartDate DESC,PDL_SequenceNo DESC";
        }

        public static string SQL_RegionDeals(string regionID)
        {
            return @"SELECT ph.str_placetitle, ph.str_placeId, PRI.PDLID, PRI.PDL_Title, PRI.PDL_SequenceNo 
               ...
               INNER Join STR_Places_Hierarchy PLH ON PLH.STR_PlaceID = PRI.PDL_Gateway AND PLH.STR_UserID= " + regionID + @" AND PLH.STR_PlaceActive = 1 AND PLH.STR_NoWeb = 0 AND PLH.STR_ProdKindID = 0 
               inner Join 
               (select str_placetitle, str_placeId, strid, str_UserId from STR_Places_Hierarchy ph 
               where ph.STR_UserID= " + regionID + @" and ph.STR_NoWeb = 0 and ph.STR_PlaceActive = 1 and str_placetypeId in (5, 23) and ph.Str_ProdKindId = 0
               ) ph on(ph.STR_PlaceID = PLH.STR_Place1ParentID OR ph.STR_PlaceID = PLH.STR_Place2ParentID)
               WHERE SPR.STP_UserID = " + regionID + @" And SPR.STP_Active = 1 And SPR.STP_StartDate <= Convert(VARCHAR(10), GETDATE(), 101) And SPR.STP_EndDate >= Convert(VARCHAR(10), GETDATE(), 101) 
        ...
        public static string SQL_Get_NumberOfCustomerFeedbacks_Per_OverAllScore()
        {
            return @"SELECT isnull(convert(varchar(5),PCC_OverallScore),'Total') as Name, count(CF.PCCID) as Id 
              FROM PRD_CustomerComment cf 
              INNER JOIN RSV_Heading CFH with (nolock) ON CF.PCC_BookingID = CFH.ID
              WHERE CF.PCC_Active = 1 and PCC_Block=0 And PCC_OverallScore Is Not null and CF.PCC_DetailID = 0 
        ...
        public static string SQL_ReviewPage(string revID)
        {
            return @"Select CF.PCCID 
                ...
                , (select top 1 STR_UserID
                from PRD_Product PRO
                INNER JOIN STR_Places_Hierarchy CONT ON (CONT.STR_PlaceID = PRO.SPD_CountryPlaceID) AND (CONT.STR_PlaceActive = 1) AND (CONT.STR_NoWeb = 0) and (CONT.STR_UserID in (182,595,243))
                WHERE PRO.SPDID = PRI.PDL_ProductID) as STR_UserId
                FROM PRD_CustomerComment CF 
                INNER JOIN RSV_Heading CFH with (nolock) ON CF.PCC_BookingID = CFH.ID
                LEFT JOIN PRD_CustomerComment_PrePublished CFP ON CF.PCC_BookingID = CFP.TCF_BookingID
        ...
        public static string SQL_ReviewsPerPage(string page, string score)
        {
            if (score == "0")
            {
                score = "1,2,3,4,5";
            }
            return @"With Feeds (PCCID, N) As  
                (Select CF.PCCID, row_number() Over(ORDER BY CFP.TCF_FeedbackReceivedTime DESC) As N FROM PRD_CustomerComment CF
                LEFT JOIN PRD_CustomerComment_PrePublished CFP On CF.PCC_BookingID = CFP.TCF_BookingID 
                WHERE CF.PCC_Active= 1 And CF.PCC_Block = 0  And CF.PCC_DetailID = 0 And LEN(cast(CF.PCC_Comment As varchar(8000))) > 15 
                And CF.PCC_OverallScore in (" + score + @")) 
                Select f.*, CONVERT(varchar(10), CFP.TCF_FeedbackReceivedTime, 101)As TCF_FeedbackReceivedTime, CONVERT(int,CFH.dept) As dept, CFH.dep_date, CF.PCC_Comment, CF.PCC_PDLID, CF.PCC_Itinerary 
                ...
                ,NoOfFeed.Value As NoOfFeed, RelatePlaces.Value As RelatePlaces, CountryNA.Value As CountryNA, STR_UserId.Value as STR_UserId, Total.Value As Total 
               from Feeds f 
               inner join PRD_CustomerComment CF On CF.PCCID=f.PCCID 
               inner join RSV_Heading CFH With (nolock) On CF.PCC_BookingID = CFH.ID 
               ...
               outer apply (Select (Select DISTINCT ph2.STR_PlaceTitle + '|' + convert(varchar(8),ph2.STR_PlaceID) + '|' + convert(varchar(8),ph2.STR_PlaceTypeID) + '@ ' 
                            FROM STR_Places_Hierarchy ph2 
                             WHERE charindex(','+convert(varchar(8),ph2.STR_PlaceID)+',', ','+replace(PDL_Places,' ',''))>0 
                                 AND ph2.STR_PlaceActive = 1 AND ph2.STR_NoWeb = 0 AND ph2.STR_PlaceTypeID in (1,25,5) AND ph2.STR_UserID in(243,182,595) 
                           FOR XML PATH('')) as Value) RelatePlaces 
               outer apply (Select top 1 STR_PlaceTitle as Value FROM PRD_Product PRO 
                            INNER JOIN STR_Places_Hierarchy CONT ON (CONT.STR_PlaceID = PRO.SPD_CountryPlaceID) AND (CONT.STR_PlaceActive = 1) AND (CONT.STR_NoWeb = 0) 
                            WHERE PRO.SPDID = PRI.PDL_ProductID) as CountryNA 
               outer apply (Select top 1 STR_UserID as Value FROM PRD_Product PRO 
                            INNER JOIN STR_Places_Hierarchy CONT ON (CONT.STR_PlaceID = PRO.SPD_CountryPlaceID) AND (CONT.STR_PlaceActive = 1) AND (CONT.STR_NoWeb = 0) and (CONT.STR_UserID in (182,595,243))
                            WHERE PRO.SPDID = PRI.PDL_ProductID) as STR_UserId 
               outer apply (Select count(*) as Value from Feeds) as Total 
               where f.N between ((" + page + @"-1)*10)+1 and " + page + @"*10 Order by f.N";
        ...
        public static string SQL_CountryPlaces(string countryId)
        {
            return @"SELECT ctry.STR_PlaceTitle, city.STR_PlaceID as CityID, city.STR_PlaceTitle as CityName, city.STR_PlaceTypeID as CityType
                      ,case city.STR_UserId when 243 then 'europe' when 182 then 'latin' when 595 then 'asia' end as CityDept, isnull(city.STR_PlaceShortInfo,'none') as CityInfo
                ...
                WHERE ctry.STR_PlaceId=" + countryId + @" AND ctry.STR_PlaceActive = 1 AND ctry.STR_NoWeb = 0 AND ctry.STR_ProdKindID = 0 AND ctry.STR_PlacePriority=1 and ctry.STR_UserID in (243,595,182)
                ORDER BY city.STR_PlaceTitle";
        }

        public static string SQL_CountryPlacesOnly(string countryId)
        {
            return @"Select isnull(ph1.STR_PlaceID,-1) as CityId, ph1.STR_PlaceTitle as CityName, ph1.STR_PlaceAIID as CityRank, ph1.STR_PlaceTypeID as CityType
                ,case ph1.STR_UserId when 243 then 'europe' when 182 then 'latin' when 595 then 'asia' end as CityDept, isnull(ph1.STR_PlaceShortInfo,'none') as CityInfo
                from STR_Places_Hierarchy ph1
                Where(ph1.STR_Place2ParentID = " + countryId + @" Or ph1.STR_Place1ParentID = " + countryId + @") and ph1.STR_UserID in (243,595,182)
                AND ph1.STR_PlaceTypeID in (1,25,6)
                And ph1.STR_PlaceActive = 1 
                And ph1.STR_NoWeb = 0
                and ph1.STR_PlacePriority = 1
                and ph1.STR_PlaceAIID > 3
                Order by CityName";
        }
        ...
        public static string SQL_GetPlacesFromSTR(string plcIds)
        {
            ...
            WHERE p1.STR_UserID =  243  And p1.STR_PlaceActive = 1 AND p1.STR_PlaceTypeID in (1, 25, 6, 2) AND p1.STR_NoWeb = 0 
                ...
                AND p2.STR_PlaceTypeID = 5 AND p2.STR_PlaceActive = 1 AND p1.STR_PlaceTitle not like 'zz%' AND p1.STR_PlaceTitle not like 'zPend%'
        ...
        //Added by Andrei
        //Mobile usage
        public static string SQL_GetAllPacksCountry(string placeId)
        {
        ...
        public static string SQL_Activities(string Ids, string OrderValue)
        {
            string sqlCom = @"SELECT pri.PDLID as Id, pri.pdl_title as Name, PDL_SequenceNo, isnull(STPR.STP_Save,9999) as STP_Save, SYC.SCD_CodeTitle 
                ,case when STPR.STP_NumOfNights is null then PRI.PDL_Duration else STPR.STP_NumOfNights end as Duration, SYC1.SCD_CodeTitle as DurationUnit 
                , VIA.PIDD_Rating As Rating
                ,VIA.PIDD_ReviewCount as Reviews
        ...
        public static string SQL_Country_Top3MostPopularPackages(string countryid)
        {
            return @"SELECT TOP 3 PRI.PDLID, PRI.PDL_Title, cast(isnull(SPR.STP_Save,9999) as money) as STP_save 
               ...
               FROM PRD_PlaceXProductItem PXP with (index(IX_PRD_PlaceXProductItem)) 
        ...
        {
            return @"Select PRI.PDLID, PRI.PDL_Title, PRI.PDL_Places, cast(isnull(SPR.STP_Save,9999) as money) as STP_save, PLH.STR_PlaceTitle as CountryName ,PLH.STR_PlaceID as CountryId 
                From PRD_ProductItem PRI 
                INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID 
                LEFT JOIN STR_SitePromotion SPR ON SPR.STP_ProdItemID = PRI.PDLID 
                AND (SPR.STP_UserID =  243 ) 
        ...
        public static string SQL_CMSContent(Int32 CMSIds)
        {
            return @"Select * From CMS_WebsiteContent Where CMS_Active = 1 AND CMSID = " + CMSIds;
        }
        public static string SQL_CMSContentByDesc(string name)
        {
            return @"Select * From CMS_WebsiteContent Where CMS_Active = 1 AND CMS_Description like '" + name + "'";
        }
        ...
        public static string SQL_CheckBookingSurvery(Int64 bookingId, string email)
        {
            return @"select RSV_Active, ID as BookingId, txt1 as EmailAddress from RSV_Heading where ID =" + bookingId + " and txt1 like '" + email + "%'";
        }
        public static string SQL_PostBookingSurvey(Int64 bookingId, string email)
        {
            return @"select * from CUST_AfterPurchaseSurvey where CAPS_BookingID = " + bookingId + " and CAPS_EmailAddress like '" + email + "'";
        }
        ...
        {
            return @"SELECT dept As Id, txt1 as Name
                FROM RSV_Heading 
                WHERE ID = " + bookID;
        }
        ...
        public static string MySQL_RecentlyViewed(string VisitorId)
        {
            return @"set @row_number := 0;
                ...
                Where t1.num in (0, 1) Group by UTS_ProductItemID ORDER BY UTS_Date  Desc Limit 20; ";
        }

        public static string MySQL_HeaderRecentlyViewed(string VisitorId)
        {
            return @"set @row_number := 0;
                 select t1.UTS_ProductItemID, t1.UTS_URL, Max(t1.UTS_Date) UTS_Date, t1.UTS_Site, t1.num 
                from
            ...
            return @"set @row_number := 0;
               ...
               (
                ...
                ORDER BY UTS_Date Desc
                      ...
                      ORDER BY UTS_Date  Desc Limit 20";
        }
        ...
        public static string SQL_CountryHighlights(string placeID)
        {
            return @"Select PLD.STX_Title
                  ...
                  From STR_PlaceDescription PLD
                  Where(STX_UserID = 243 )
                  AND PLD.STX_Active = 1 AND PLD.STX_MasterContentID = 0 AND PLD.STX_StrId = " + placeID +
                  @"ORDER BY PLD.STX_ProdKindID, PLD.STX_Priority";
        }
        ...
        public static string SQL_AllCountryDestinationsByDepartID()
        {
            return @"Select 
                Case
                When PLC.STR_UserID = 595 Then 'TMAS'
                When PLC.STR_UserID = 182 then 'LD'
                When PLC.STR_UserID = 243 then 'ED'
                ELSE ''
                END as DepartNA
                ,PLC.STR_PlaceID as CountryID
                ,PLC.STR_PlaceTitle as CountryNA
                ,isnull(PLC.STR_PlaceAIID,1000) as CountryRK
                FROM STR_Places_Hierarchy PLC
                WHERE PLC.STR_PlaceActive = 1 
                AND PLC.STR_NoWeb = 0 
                AND PLC.STR_UserID IN (182,243,595) 
                AND PLC.STR_PlaceTypeID in (5,23)
                ORDER BY DepartNA , CountryRK ASC, CountryNA DESC";
        }

        public static string SQL_HeaderMostPopItineraries()
        {
            return @";with promo as
                (select STP.STP_UserID, STP.STP_NumOfNights, STP_ProdItemID, STP.STP_Save from STR_SitePromotion STP 
                where STP_ProdItemID in (" + defaultMostPop + @") AND STP.STP_UserID in (243,595,182) AND STP.STP_Active = 1 AND STP.STP_StartDate <= CONVERT(VARCHAR(10),GETDATE(),101) AND  STP.STP_EndDate >= CONVERT(VARCHAR(10),GETDATE(),101) ) 
                ,pack as 
                (Select cf.PCC_Comment, h.dep_date, PRI.PDLID, PRI.PDL_Title, PRI.PDL_Duration, PRO.SPD_CountryPlaceID, img.IMG_Path_URL, img.IMG_500Path_URL
                ,row_number() OVER(Partition by cf.PCC_PDLID Order by dep_Date DESC) as pk 
                From PRD_CustomerComment CF 
                Inner Join RSV_Heading H with (nolock) on ID = CF.PCC_BookingID AND H.dep_date > convert(Varchar(10),Getdate()- 730,101) 
                inner join PRD_ProductItem PRI on PRI.PDLID = CF.PCC_PDLID 
                INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID
                LEFT JOIN PRD_ProductXImages PxImg ON PxImg.PXI_ProductID = PRI.PDL_ProductID AND (PxImg.PXI_Sequence = 0) AND (PxImg.PXI_Active = 1) 
                LEFT JOIN APP_Images Img ON Img.IMGID = PxImg.PXI_ImageID and Img.IMG_Active=1 
                Where CF.PCC_PDLID in (" + defaultMostPop + @") AND cf.PCC_Comment not like ('%---%') AND CF.PCC_Active = 1 AND CF.PCC_Block = 0 AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 15 
                    AND PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 AND PRO.SPD_Active = 1 AND PRO.SPD_Producttypesyscode=34 AND H.RSV_ACtive = 1 )
                select p.PDLID, p.PDL_Title,isnull(try_Convert(money,pr.STP_Save),99999) as STP_Save 
                    ,CASE WHEN pr.STP_NumOfNights is null then p.PDL_Duration ELSE pr.STP_NumOfNights END as Duration 
                    , CONT.STR_PlaceTitle as CountryNA, p.IMG_Path_URL 
                  ,CASE When pr.STP_UserID = 595 then 'TMAS' When pr.STP_UserID = 182 then 'LD' When pr.STP_UserID = 243 then 'ED' else '' end as deptNA 
                  ,CASE When pr.STP_UserID = 595 then 2 When pr.STP_UserID = 182 then 3 When pr.STP_UserID = 243 then 1 else 99 end as Ord 
                From pack p 
                inner join promo pr on pr.STP_ProdItemID=p.PDLID 
                INNER JOIN STR_Places_Hierarchy CONT ON (CONT.STR_PlaceID = p.SPD_CountryPlaceID) AND (CONT.STR_PlaceActive = 1) AND (CONT.STR_UserID = pr.STP_UserID) AND (CONT.STR_NoWeb = 0) 
                Where pk = 1 ORDER BY Ord";
        }
        public static string SQL_MostPopItineraries()
        {
            return @";with promo as
                (select STP.STP_UserID, STP.STP_NumOfNights, STP_ProdItemID, STP.STP_Save from STR_SitePromotion STP 
                where STP_ProdItemID in (" + defaultMostPop + @") AND STP.STP_UserID in (243,595,182) AND STP.STP_Active = 1 AND STP.STP_StartDate <= CONVERT(VARCHAR(10),GETDATE(),101) AND  STP.STP_EndDate >= CONVERT(VARCHAR(10),GETDATE(),101) ) 
                ...
                Where CF.PCC_PDLID in (" + defaultMostPop + @") AND cf.PCC_Comment not like ('%---%') AND CF.PCC_Active = 1 AND CF.PCC_Block = 0 AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 15 
                    AND PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 AND PRO.SPD_Active = 1 AND PRO.SPD_Producttypesyscode=34 AND H.RSV_ACtive = 1 )
                select p.dep_date, p.PDLID, p.PDL_Title,isnull(try_Convert(money,pr.STP_Save),99999) as STP_Save 
                    ,CASE WHEN pr.STP_NumOfNights is null then p.PDL_Duration ELSE pr.STP_NumOfNights END as Duration 
                    ,p.SPD_CountryPlaceID, CONT.STR_PlaceTitle as CountryNA, p.IMG_Path_URL,p.IMG_500Path_URL, p.PCC_Comment as Comment, p.dep_date 
                  ,CASE When pr.STP_UserID = 595 then 'TMAS' When pr.STP_UserID = 182 then 'LD' When pr.STP_UserID = 243 then 'ED' else '' end as deptNA 
                  ,CASE When pr.STP_UserID = 595 then 2 When pr.STP_UserID = 182 then 3 When pr.STP_UserID = 243 then 1 else 99 end as Ord 
                From pack p 
                inner join promo pr on pr.STP_ProdItemID=p.PDLID 
        ...
        public static string SQL_FeaturedPacksByDepartID()
        {
            return @"Select PRI.PDLID 
                ,PRI.PDL_Title
                ,PRI.PDL_Content as Included 
                ,PRI.PDL_SequenceNo
                ,STP.STP_UserID
                ,CASE 
                When STP.STP_UserID = 595 then 'TMAS'
                When STP.STP_UserID = 182 then 'LD'
                When STP.STP_UserID = 243 then 'ED'
                else ''
                end as deptNA
                ,isnull(try_Convert(money,STP.STP_Price),99999) as STP_Price
                ...
                ,PRO.SPD_CountryPlaceID as CountryID
                ,(Select top 1 PLCO.STR_PlaceTitle  FROM STR_Places_Hierarchy PLCO where PLCO.STR_PlaceID = PRO.SPD_CountryPlaceID and PLCO.STR_PlaceActive = 1 AND PLCO.STR_NoWeb = 0) as CountryName
                ,IMG.IMG_Path_URL,isnull(IMG.IMG_500Path_URL,'none') as IMG_500Path_URL, IMG.IMG_Title
                ,(SELECT COUNT (CF.PCCID) as NoOfFeed FROM PRD_CustomerComment CF  
                     INNER JOIN RSV_Heading CFH with (nolock) ON CF.PCC_BookingID = CFH.ID  
                     WHERE(CF.PCC_PDLID = PRI.PDLID)
                     AND CF.PCC_Comment is not null AND LEN(cast(CF.PCC_Comment as varchar(8000))) > 20  
                     AND CF.PCC_Active = 1 AND CF.PCC_Block = 0) as NoOfFeed 
                FROM STR_SitePromotion STP
                INNER JOIN PRD_ProductItem PRI ON PRI.PDLID = STP.STP_ProdItemID
                INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID
            ...
            WHERE 
                ...
                AND PRO.SPD_Active = 1 
                AND PRI.PDL_Active = 1 
                AND PRI.PDL_SequenceNo < 10 
                AND PRI.PDL_NoWeb = 0 
                AND STP.STP_TypePromotion = 1 
                ORDER BY deptNA, Case When STP_TypePromotion = 0 Then 99 else STP_TypePromotion End ASC, STP_StartDate DESC,PDL_SequenceNo DESC";
        }

        public static string SQL_VacationPackages()
        {
            return @"Select PRI.PDLID 
                ,PRI.PDL_Title
                ,PRI.PDL_Content as Included 
                ...
                LEFT JOIN APP_Images IMG ON IMG.IMGID = PXI.PXI_ImageID 
                AND IMG.IMG_Active = 1
                LEFT JOIN PRD_Place PLC ON PLC.PLCID = STP.STP_FromPlaceID
                AND (PLC.PLC_PlaceTypeID = 1) AND (PLC.PLC_Active = 1)
            WHERE 
                STP.STP_UserID IN (182,243,595) 
                AND STP.STP_Active = 1 
        ...
        public static string SQL_Region_FeaturedPacks(string RegionID)
        {
            return @"SELECT PRI.PDLID, PRI.PDL_Title
                    FROM STR_SitePromotion STP 
                    INNER JOIN PRD_ProductItem PRI ON PRI.PDLID = STP.STP_ProdItemID and PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 
                    INNER JOIN PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID 
                    WHERE STP.STP_UserID = " + RegionID + @" And STP.STP_Active = 1 AND STP.STP_TypePromotion = 1 AND PRI.PDL_NoWeb = 0 and PRI.PDL_Active = 1 
                        AND STP.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101) AND STP.STP_EndDate >= CONVERT(VARCHAR(10),GETDATE(),101) 
                        order by STP.STP_StartDate DESC 
                        ,case when PDL_SequenceNo = 99 then 99 when PDL_SequenceNo < 8 then 99 - PDL_SequenceNo when PDL_SequenceNo between 8 and 90 then PDL_SequenceNo end asc";
        }

        public static string SQL_PackageCustomInformation(string PackId)
        {
            return @"select spr.STP_UserId as RegionId, PRI.PDLID as Id, PRI.PDL_Title as Name,PRI.PDL_Content as Content, PRO.SPD_Description as Description, IMG.IMG_Path_URL 
               ,(Select top 1 PLCO.STR_PlaceTitle FROM STR_Places_Hierarchy PLCO where PLCO.STR_PlaceID = PRO.SPD_CountryPlaceID and PLCO.STR_PlaceActive = 1 AND PLCO.STR_NoWeb = 0) as CountryName 
               ,case when SPR.STP_NumOfNights is null then PRI.PDL_Duration else SPR.STP_NumOfNights end as Duration 
               ,cast(isnull(SPR.STP_Save,9999) as Money) as Price 
               ,(SELECT count(CF.PCCID) FROM PRD_CustomerComment CF WHERE CF.PCC_PDLID = " + PackId + @" AND datalength(CF.PCC_Comment) > 15 AND CF.PCC_Active = 1 AND CF.PCC_Block = 0) as NoOfFeeds 
               from PRD_ProductItem PRI 
               inner Join PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID AND PRO.SPD_Active = 1 AND PRO.SPD_ProductTypeSysCode = 34 AND PRO.SPD_StarratingSysCode <> 541 
               left Join STR_SitePromotion SPR with (nolock) ON SPR.STP_ProdItemID =PRI.PDLID AND SPR.STP_Active = 1 
                 AND SPR.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101) AND SPR.STP_EndDate >= CONVERT(VARCHAR(10), GETDATE(), 101) 
               ...
               where PRI.PDLID = " + PackId + @" and PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 AND pri.PDL_Title NOT LIKE 'Zpend%'";
        }

        public static string SQL_PackageRelatedDestinations(string PackId, string RegionId)
        {
            string internalComms = "";
            switch (RegionId)
            {
                case "595":
                    internalComms = ":.TW";
            ...
            return @"select pxp.cxz_ChildPlaceId as Id, ph.str_placetitle as Name 
               from prd_placexproductitem pxp 
               inner join str_places_hierarchy ph ON ph.str_placeid=pxp.cxz_childplaceid AND ph.str_userid=" + RegionId + @" and ph.str_placetypeid <> 3 
                     and ph.str_placeActive = 1 and ph.Str_Noweb=0 and ph.str_ProdkindId=0 
               where pxp.cxz_productitem = " + PackId + @" And pxp.cxz_active = 1";
        }

        public static string SQL_AllTopDeals()
        {
            return @"Select PRI.PDLID, PRI.PDL_Title, PRI.PDL_SequenceNo, STP.STP_UserID 
               , isnull(try_Convert(money,STP.STP_Save),99999) as STP_Save 
               ,case when STP.STP_NumOfNights is null then PRI.PDL_Duration else STP.STP_NumOfNights end as STP_NumOfNights 
               ...
               left join PRD_ProductXImages PXI ON PXI.PXI_ProductID = PRO.SPDID AND PXI.PXI_Sequence = 0 AND PXI.PXI_Active = 1
               left join APP_Images IMG ON IMG.IMGID = PXI.PXI_ImageID AND IMG.IMG_Active = 1 AND IMG.IMG_Path_URL is not null
               WHERE STP.STP_UserID IN (243,182,595) AND STP.STP_Active = 1 and  STP_TypePromotion = 1 
                 AND STP.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101) AND STP.STP_EndDate >= CONVERT(VARCHAR(10), GETDATE(), 101) 
                 AND PRO.SPD_Producttypesyscode = 34 AND PRI.PDL_SequenceNo < 10 
               ORDER BY Case STP_UserID when 243 then 1 when 595 then 2 when 182 then 3 end, Case When STP_TypePromotion = 0 Then 99 else STP_TypePromotion End ASC, STP_StartDate DESC,PDL_SequenceNo DESC";
        }

        public static string SQL_AllRegionDeals()
        {
            return @"SELECT ph.str_placetitle, ph.str_placeId, PRI.PDLID, PRI.PDL_Title, PRI.PDL_SequenceNo  
               ,cast(isnull(SPR.STP_Save,9999) as Money) as STP_Price, case when SPR.STP_NumOfNights is null then PRI.PDL_Duration else SPR.STP_NumOfNights end as STP_NumOfNights, ph.STR_UserID  
               FROM STR_SitePromotion SPR  
               INNER JOIN PRD_ProductItem PRI ON SPR.STP_ProdItemID = PRI.PDLID AND PRI.PDL_Active = 1 AND PRI.PDL_NoWeb = 0 AND pri.PDL_Title NOT LIKE 'Zpend%'  
               INNER Join STR_Places_Hierarchy PLH ON PLH.STR_PlaceID = PRI.PDL_Gateway AND PLH.STR_UserID in (243,595,182)  AND PLH.STR_PlaceActive = 1 AND PLH.STR_NoWeb = 0 AND PLH.STR_ProdKindID = 0 AND PLH.STR_PlacePriority = 1  
               inner Join  
               (select str_placetitle, str_placeId, strid, str_UserId from STR_Places_Hierarchy ph  
               where ph.STR_UserID in (243,595,182)  and ph.STR_NoWeb = 0 and ph.STR_PlaceActive=1 and str_placetypeId in (5,23) and ph.Str_ProdKindId=0 and ph.STR_PlacePriority = 1 
               ) ph on (ph.STR_PlaceID = PLH.STR_Place1ParentID OR ph.STR_PlaceID = PLH.STR_Place2ParentID)  
               WHERE SPR.STP_UserID in (243,595,182)  And SPR.STP_Active = 1 And SPR.STP_StartDate <= Convert(VARCHAR(10), GETDATE(), 101) And SPR.STP_EndDate >= Convert(VARCHAR(10), GETDATE(), 101)  
               order by ph.str_placetitle, PRI.PDL_SequenceNo, PRI.PDLID";
        }
        //New query for Top Deals Responsive
        //Andrei
        //2024/11/20
        public static string SQL_GetAllTopDeals()
        {
            return @"exec dbo.WEB_TMGetTopDealsPackages";
        }
        public static string SQL_Destinations()
        {
            return @"select ph.STR_USERID as RegionId, ph.STR_PlaceID as CountryId, ph.STR_PlaceTitle as CountryName, Cities.CityId, Cities.CityName, Cities.CityRank
                from STR_Places_Hierarchy ph
                outer apply (select top 4 ph1.STR_PlaceID as CityId, ph1.STR_PlaceTitle as CityName, isnull(ph1.STR_PlaceAIID,9999) as CityRank, ph1.STR_PlaceTypeID as CityType 
                             from STR_Places_Hierarchy ph1 where (ph1.STR_Place2ParentID = ph.STR_PlaceID Or ph1.STR_Place1ParentID = ph.STR_PlaceID) and ph.STR_UserID=ph1.STR_UserID 
                                  AND ph1.STR_PlaceTypeID in (1,25) And ph1.STR_PlaceActive = 1 And ph1.STR_NoWeb = 0 and ph1.[STR_PlacePriority]=1 order by isnull(ph1.STR_PlaceAIID,9999)) Cities
                where ph.STR_UserID in (243,595,182) AND ph.STR_PlaceTypeID in (5,23) and ph.Str_ProdKindId=0 AND ph.STR_PlaceActive = 1 AND ph.STR_NoWeb = 0 and ph.[STR_PlacePriority]=1
                order by ph.STR_PlaceTitle";
        }
        public static string SQL_DestinationsPlusInfo()
        {
            return @"select ph.STR_USERID as RegionId, ph.STR_PlaceID as CountryId, ph.STR_PlaceTitle as CountryName, isnull(ph.STR_PlaceShortInfo, ph.STR_PlaceTitle) as CountryInfo, isnull(Cities.CityId,-1) as CityId, isnull(Cities.CityName,'none') as CityName, isnull(Cities.CityInfo,'none') as CityInfo, isnull(Cities.CityRank, -1) as CityRank
            , (Select count(*) From STR_Places_Hierarchy ph3
            where (ph3.STR_Place2ParentID = ph.STR_PlaceID Or ph3.STR_Place1ParentID = ph.STR_PlaceID) and ph.STR_UserID=ph3.STR_UserID 
            AND ph3.STR_PlaceTypeID in (1,25,6) 
            And ph3.STR_PlaceActive = 1 
            And ph3.STR_NoWeb = 0 
            and ph3.STR_PlacePriority = 1) as CitiesTotal
            from STR_Places_Hierarchy ph
            outer apply (select isnull(ph1.STR_PlaceID,-1) as CityId, ph1.STR_PlaceTitle as CityName, ph1.STR_PlaceShortInfo as CityInfo, ph1.STR_PlaceAIID as CityRank, ph1.STR_PlaceTypeID as CityType 
            from STR_Places_Hierarchy ph1 
            where (ph1.STR_Place2ParentID = ph.STR_PlaceID Or ph1.STR_Place1ParentID = ph.STR_PlaceID) and ph.STR_UserID=ph1.STR_UserID 
            AND ph1.STR_PlaceTypeID in (1,25,6) 
            And ph1.STR_PlaceActive = 1 
            And ph1.STR_NoWeb = 0 
            and ph1.STR_PlacePriority = 1
            and ph1.STR_PlaceAIID < 4               
            ) Cities
            where ph.STR_UserID in (243,595,182) 
            AND ph.STR_PlaceTypeID in (5,23) 
            and ph.Str_ProdKindId=0 
            AND ph.STR_PlaceActive = 1 
            AND ph.STR_NoWeb = 0 
            and ph.STR_PlacePriority = 1
            order by ph.STR_PlaceTitle, CityRank";
        }
        public static string SQL_ContactAgent(string email, int agentId)
        {
            return @"select USR_Lastname as LastName, USR_Firstname as FirstName, TAST_AgentUsername as AgentEmail, TAST_UserID as AgentID, TAST_Status as AgentStatus, TAST_ContactType as ContactType, TAST_ActiveChats as ActiveChats, SCDID as DepartmentID, SCD_CodeTitle as DepartmentName,
            convert(datetime,HRSUS_MonFirstInTime) as MondayIn, convert(datetime,HRSUS_MonSecondOutTime) as MondayOut, convert(datetime,HRSUS_TueFirstInTime) as TuesdayIn, convert(datetime,HRSUS_TueSecondOutTime) as TuesdayOut,
            convert(datetime,HRSUS_WedFirstInTime) as WednesdayIn, convert(datetime, HRSUS_WedSecondOutTime) as WednesdayOut, convert(datetime, HRSUS_ThuFirstInTime) as ThursdayIn, convert(datetime, HRSUS_ThuSecondOutTime) as ThursdayOut,
            convert(datetime, HRSUS_FriFirstInTime) as FridayIn, convert(datetime, HRSUS_FriSecondOutTime) as FridayOut, convert(datetime, HRSUS_SatFirstInTime) as SaturdayIn, convert(datetime, HRSUS_SatSecondOutTime) as SaturdayOut,
            convert(datetime, HRSUS_SunFirstInTime) as SundayIn, convert(datetime, HRSUS_SunSecondOutTime) as SundayOut
                from TEL_CCPUserStatus
                inner join App_User on TAST_UserID = UsrID 
                inner join SYS_Codes on USR_EmployeeDivisionID = SCDID
                inner join HRS_UserSchedules on HRSUS_UserID = UsrID
            where SCDID in (2181) and TAST_AgentUsername like '%" + email + @"%' and TAST_UserID = " + agentId + @" and getdate() between HRSUS_StartDate 
            and isnull(HRSUS_EndDate, '1/1/2030') and HRSUS_Active = 1";
        }

        public static string SQL_GetChatAgent(string email)
        {
            return @"select USR_Lastname as LastName, USR_Firstname as FirstName, TAST_AgentUsername as AgentEmail, TAST_UserID as AgentID, TAST_Status as AgentStatus, SCDID as DepartmentID, SCD_CodeTitle as DepartmentName,
            TAST_ContactType as ContactType, TAST_ActiveChats as ActiveChats,
            convert(datetime,HRSUS_MonFirstInTime) as MondayIn, convert(datetime,HRSUS_MonSecondOutTime) as MondayOut, convert(datetime,HRSUS_TueFirstInTime) as TuesdayIn, convert(datetime,HRSUS_TueSecondOutTime) as TuesdayOut,
            convert(datetime,HRSUS_WedFirstInTime) as WednesdayIn, convert(datetime, HRSUS_WedSecondOutTime) as WednesdayOut, convert(datetime, HRSUS_ThuFirstInTime) as ThursdayIn, convert(datetime, HRSUS_ThuSecondOutTime) as ThursdayOut,
            convert(datetime, HRSUS_FriFirstInTime) as FridayIn, convert(datetime, HRSUS_FriSecondOutTime) as FridayOut, convert(datetime, HRSUS_SatFirstInTime) as SaturdayIn, convert(datetime, HRSUS_SatSecondOutTime) as SaturdayOut,
            convert(datetime, HRSUS_SunFirstInTime) as SundayIn, convert(datetime, HRSUS_SunSecondOutTime) as SundayOut
                ...
                inner join SYS_Codes on USR_EmployeeDivisionID = SCDID
                inner join HRS_UserSchedules on HRSUS_UserID = UsrID
            where SCDID in (2181, 2019, 2158) 
            and TAST_AgentUsername like '%" + email + @"%'
            and getdate() between HRSUS_StartDate and isnull(HRSUS_EndDate, '1/1/2030') and HRSUS_Active = 1";
        }


        public static string SQL_TaxCityInfo()
        {
            return @"Select tx.*, ctr.STR_UserID
               From PRD_CityTaxInfo tx
               inner Join STR_Places_Hierarchy ctr on ctr.STR_PlaceID = tx.PCTI_CountryID 
               and ctr.STR_PlaceActive = 1 
               and ctr.STR_PlacePriority = 1 
               and ctr.STR_PlaceTypeID = 5 
               and ctr.STR_NoWeb = 0 
               and ctr.STR_UserID in (182,243,595)
               Where tx.PCTI_Active = 1 
               Order by ctr.STR_UserID,tx.PCTI_CountryName";
        }
        public static string SQL_GetCarInfo(string brand, string model)
        {
            return @"Select *,(IAEM_Brand + IAEM_Model)as Combo FROM IAEAPI_CarModels WHERE IAEM_Active = 1 AND IAEM_Brand LIKE '" + brand + @"' AND IAEM_Model LIKE '" + model + @"'";
        }

        public static string SQL_NoteworthyPacks(string pckIDs)
        {
            return @"Select PRI.PDLID, PRI.PDL_Title, PEX.PDLX_IsLuxury, STP.STP_UserID, STP.STP_NumOfNights, STP.STP_Save, STP.STP_UserID, PRO.SPD_CountryPlaceID as CountryID
                , (Select top 1 PLCO.STR_PlaceTitle  FROM STR_Places_Hierarchy PLCO where PLCO.STR_PlaceID = PRO.SPD_CountryPlaceID and PLCO.STR_PlaceActive = 1 AND PLCO.STR_NoWeb = 0) as CountryName 
                , Case When img.IMG_500Path_URL is null then img.IMG_Path_URL  else img.IMG_500Path_URL end as IMG_500Path_URL
                From PRD_ProductItem PRI
                left join PRD_Product PRO ON PRO.SPDID = PRI.PDL_ProductID
                left join PRD_ProductItemExtraProperty PEX on PEX.PDLX_PDLID = PRI.PDLID
                left join STR_SitePromotion STP on STP.STP_ProdItemID = PRI.PDLID and STP.STP_Active = 1
                left join PRD_ProductXImages PxImg ON PxImg.PXI_ProductID = PRI.PDL_ProductID AND (PxImg.PXI_Sequence = 0) AND (PxImg.PXI_Active = 1) 
                left join APP_Images Img ON Img.IMGID = PxImg.PXI_ImageID and Img.IMG_Active=1 
                Where PDLID in (" + pckIDs + @")
                AND STP.STP_StartDate <= CONVERT(VARCHAR(10), GETDATE(), 101) AND STP.STP_EndDate >= CONVERT(VARCHAR(10), GETDATE(), 101)
                AND PRO.SPD_Producttypesyscode = 34 AND PRI.PDL_SequenceNo = 2
