...
namespace MVC_TM.Infrastructure
{
    ...
    public class Utilities
    {
        ...
        public static string GetUt2Values(string AllCookies)
        {
            //how to get all cookies
            //foreach (var t in HttpContext.Request.Headers)
            //{
            //    if (t.Key == "Cookie")
            //    {
            //        string AllCookies = t.Value;
            //    }
            //}

            string CookieVal = AllCookies;
            string utvId = "";
            string utlogId = "";
            ...
            try
            {
                Regex r = new Regex("(?<=ut2=).*?(?=;)", RegexOptions.None);
                Match mc = r.Match(CookieVal);
                if (mc.Success)
                {
                    Regex r1 = new Regex("(?<=_utvId=).*?(?=&)", RegexOptions.None);
                    Match mc1 = r1.Match(mc.Value);
                    if (mc1.Success)
                    {
                        utvId = mc1.Value;
                    }

                    Regex r2 = new Regex("(?<=_utlogId=).*?(?=&)", RegexOptions.None);
                    Match mc2 = r2.Match(mc.Value);
                    if (mc2.Success)
                    {
                        utlogId = mc2.Value;
                    }
            ...
            {
                throw new Exception(" Detail: GetUt2Values() " + e.Message + "<br/>Error occurred.</div>");
            }

            return utvId + "|" + utlogId;
        }
